...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Adaptive attention scale: track attention weight variance with EMA
        self.register_buffer('attn_var_ema', torch.tensor(0.1))
        self.ema_decay = 0.99
        self.min_scale = 0.1
        self.max_scale = 2.0
    ...
    def forward(self, x):
        ...
        # Manual attention computation to track attention weights
        ...
        # Apply causal mask
        att = att.masked_fill(torch.triu(torch.ones(T, T, device=att.device, dtype=torch.bool), diagonal=1), float('-inf'))
        
        # Softmax to get attention weights
        att_weights = F.softmax(att, dim=-1)
        
        # Compute variance of attention weights (measure of attention confidence)
        # High variance = sharp attention, Low variance = diffuse attention
        if self.training:
            with torch.no_grad():
                # Compute variance across the key dimension for each query
                att_var = att_weights.var(dim=-1).mean()
                # Update EMA of attention variance
                self.attn_var_ema = self.ema_decay * self.attn_var_ema + (1 - self.ema_decay) * att_var
        
        # Adaptive scale based on attention variance
        # High variance (confident) -> higher scale, Low variance (diffuse) -> lower scale
        # Map variance [0, 0.1] roughly to scale [min_scale, max_scale]
        adaptive_scale = torch.clamp(
            self.min_scale + (self.attn_var_ema / 0.05) * (self.max_scale - self.min_scale),
            self.min_scale,
            self.max_scale
        )
        ...
        # Apply adaptive scale to attention output
        y = y * adaptive_scale
...
class Block(nn.Module):
    ...
    def forward(self, x):
        # Note: attn now has its own adaptive scaling, so we apply the fixed scale here
