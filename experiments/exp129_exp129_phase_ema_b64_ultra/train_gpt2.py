...
if __name__ == "__main__":
    ...
    total_tokens_per_step = B_original * T * total_grad_accum
    new_grad_accum_per_gpu = total_tokens_per_step // (B * T * ddp_world_size)
    if new_grad_accum_per_gpu < 1:
        new_grad_accum_per_gpu = 1
    grad_accumulation_steps = new_grad_accum_per_gpu
    actual_tokens_per_step = B * T * ddp_world_size * grad_accumulation_steps

    tokens_per_iter = actual_tokens_per_step
    print0(f"tokens per iteration: {tokens_per_iter:,} (target: {total_tokens_per_step:,})")
    print0(f"micro_batch={B}, grad_accum_per_gpu={grad_accumulation_steps}, world_size={ddp_world_size}")
    ...
    for step in range(args.num_iterations + 1):
        ...
        if do_val:
            ...
            if last_step:
                for orig, p in zip(originals, raw_model.parameters()):
                    p.data.copy_(orig)
