...
class Block(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Track utilization for adaptive skipping
        self.register_buffer('utilization_ema', torch.tensor(1.0))

    def forward(self, x, skip_prob=0.0):
        # Decide whether to skip based on utilization and skip probability
        if self.training and skip_prob > 0 and torch.rand(1).item() < skip_prob:
            # Skip this block - just pass through input
            return x
        ...
        # Measure utilization (relative change magnitude)
        if self.training:
            with torch.no_grad():
                change_norm = torch.norm(x - x_input)
                input_norm = torch.norm(x_input)
                utilization = (change_norm / (input_norm + 1e-8)).item()
                # Update EMA of utilization
                self.utilization_ema = 0.99 * self.utilization_ema + 0.01 * utilization
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, return_logits=True):
        ...
        # Compute adaptive skip probability based on training progress
        # Start with higher skip probability early, reduce over time
        # Also make skip decisions per-block based on their utilization
        max_skip_prob = 0.3  # maximum skip probability
        warmup_steps = 1000
        skip_decay_steps = 3000
        
        if self.training:
            if self.step_count < warmup_steps:
                base_skip_prob = 0.0  # No skipping during initial warmup
            elif self.step_count < warmup_steps + skip_decay_steps:
                # Gradually increase then decrease skip probability
                progress = (self.step_count - warmup_steps) / skip_decay_steps
                base_skip_prob = max_skip_prob * math.sin(progress * math.pi)
            else:
                base_skip_prob = 0.0  # Stop skipping in later training
        else:
            base_skip_prob = 0.0  # Never skip during evaluation

        for block in self.transformer.h:
            # Adjust skip probability based on block utilization
            # Blocks with lower utilization are more likely to be skipped
            if self.training and base_skip_prob > 0:
                utilization = block.utilization_ema.item()
                # If utilization is low (< 0.5), increase skip probability
                if utilization < 0.5:
                    block_skip_prob = base_skip_prob * (1.0 - utilization / 0.5)
                else:
                    block_skip_prob = 0.0
            else:
                block_skip_prob = 0.0
            
            x = block(x, skip_prob=block_skip_prob)
...
if __name__ == "__main__":
    ...
    for step in range(args.num_iterations + 1):
        ...
        # Update step count in model for adaptive skipping
        raw_model.step_count = step
