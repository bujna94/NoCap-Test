...
if __name__ == "__main__":
    ...
    tokens_per_step_per_gpu = args.batch_size * args.grad_accumulation_steps * T
    tokens_per_microstep = B * T
    grad_accum_internal = tokens_per_step_per_gpu // tokens_per_microstep
    if grad_accum_internal < 1:
        grad_accum_internal = 1
        B = tokens_per_step_per_gpu // T
        tokens_per_microstep = B * T

    actual_tokens_per_step_per_gpu = grad_accum_internal * B * T
    tokens_per_iter = actual_tokens_per_step_per_gpu * ddp_world_size
    print0(f"Internal B={B}, grad_accum_internal={grad_accum_internal}, tokens_per_iter={tokens_per_iter}")
    ...
    ema_stream = torch.cuda.Stream(device=device)
    ...
    for step in range(args.num_iterations + 1):
        ...
        for micro_step in range(grad_accum_internal):
            is_last_micro = (micro_step == grad_accum_internal - 1)

            if is_last_micro:
                with ctx:
                    _, loss = model(x, y, return_logits=False)
                    loss = loss / grad_accum_internal
                if should_log:
