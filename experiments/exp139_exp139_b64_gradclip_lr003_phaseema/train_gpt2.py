...
if __name__ == "__main__":
    ...
    # Compute effective grad_accum to maintain total tokens per step
    # Baseline: args.batch_size * T tokens per micro-step, args.grad_accumulation_steps micro-steps per GPU
    # Total tokens per step = args.batch_size * args.grad_accumulation_steps * T * ddp_world_size
    # With B=MICRO_BATCH: need effective_grad_accum such that
    # B * T * effective_grad_accum * ddp_world_size = args.batch_size * args.grad_accumulation_steps * T * ddp_world_size
    # => effective_grad_accum = (args.batch_size * args.grad_accumulation_steps) / B
    # But we need per-GPU grad_accum, and args.grad_accumulation_steps is already per-GPU
    # So: total per-GPU tokens = args.batch_size * args.grad_accumulation_steps * T
    # With B: per-GPU grad_accum = (args.batch_size * args.grad_accumulation_steps) // B
    effective_grad_accum = max(1, (args.batch_size * args.grad_accumulation_steps) // MICRO_BATCH)
    ...
    tokens_per_iter = B * T * ddp_world_size * effective_grad_accum
    print0(f"tokens per iteration: {tokens_per_iter:,}")
    print0(f"micro_batch: {B}, effective_grad_accum per GPU: {effective_grad_accum}, world_size: {ddp_world_size}")
    ...
    # Initialize EMA shadow weights after full model setup
    ...
    def get_ema_decay(it):
        if it < args.num_iterations - args.warmdown_iters:
            return EMA_DECAY_MAIN
        else:
            return EMA_DECAY_WARMDOWN
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            if is_final:
                ...
                # Swap in EMA weights and evaluate
        ...
        # Gradient clipping for stability with large batch + high LR
        torch.nn.utils.clip_grad_norm_(raw_model.parameters(), GRAD_CLIP_MAX_NORM)
