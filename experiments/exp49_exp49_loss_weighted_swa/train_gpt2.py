...
if __name__ == "__main__":
    ...
    # =========================================================================
    # Weight averaging configuration - DOES NOT MODIFY TRAINING
    # =========================================================================
    ...
    SWA_FREQ = 8  # collect every 8 steps
    swa_snapshots = []  # list of (step, state_dict_on_cpu)
    swa_losses = []  # training loss at each snapshot step

    # EMA: maintain 3 shadows with different decays
    EMA_DECAYS = [0.998, 0.999, 0.9995]
    ema_shadows = []  # list of dicts, each mapping param_name -> tensor on CPU
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            # Helper function to evaluate a given state dict
            def evaluate_weights(state_dict_cpu=None):
                """Evaluate val loss, optionally with different weights."""
                original_sd = None
                if state_dict_cpu is not None:
                    # Save current weights
                    original_sd = {}
                    for name, param in raw_model.named_parameters():
                        original_sd[name] = param.data.clone()
                    # Load candidate weights
                    for name, param in raw_model.named_parameters():
                        if name in state_dict_cpu:
                            param.data.copy_(state_dict_cpu[name].to(param.device))
            ...
            # First evaluate with current (raw) weights
            best_val_loss = evaluate_weights(None)
            best_label = "raw"

            # Only try weight averaging candidates on the final evaluation or after SWA starts
            if step >= SWA_START_STEP and len(swa_snapshots) >= 4:
                param_names = [name for name, _ in raw_model.named_parameters()]

                n_snaps = len(swa_snapshots)

                # --- Candidate 1: Uniform SWA (all snapshots) ---
                try:
                    uniform_sd = {}
                    for name in param_names:
                        stacked = torch.stack([s[1][name] for s in swa_snapshots])
                        uniform_sd[name] = stacked.mean(dim=0)
                    vl = evaluate_weights(uniform_sd)
                    if vl < best_val_loss:
                        best_val_loss = vl
                        best_label = "uniform_swa"
                    del uniform_sd
                except Exception as e:
                    print0(f"Uniform SWA eval failed: {e}")

                # --- Candidate 2: Linearly-weighted SWA (more weight to later) ---
                try:
                    linear_sd = {}
                    ...
                    for name in param_names:
                        stacked = torch.stack([s[1][name] for s in swa_snapshots])
                        linear_sd[name] = (stacked * weights.view(-1, *([1] * (stacked.ndim - 1)))).sum(dim=0)
                    vl = evaluate_weights(linear_sd)
                    if vl < best_val_loss:
                        best_val_loss = vl
                        best_label = "linear_swa"
                    del linear_sd
                except Exception as e:
                    print0(f"Linear SWA eval failed: {e}")

                # --- Candidate 3: Loss-weighted SWA (weight by inverse loss) ---
                try:
                    if len(swa_losses) == len(swa_snapshots) and len(swa_losses) > 0:
                        loss_tensor = torch.tensor(swa_losses, dtype=torch.float32)
                        # Use softmin: lower loss -> higher weight
                        # Temperature controls sharpness; use 10.0 for moderate sharpness
                        loss_weights = F.softmax(-loss_tensor * 10.0, dim=0)
                        loss_sd = {}
                        for name in param_names:
                            stacked = torch.stack([s[1][name] for s in swa_snapshots])
                            loss_sd[name] = (stacked * loss_weights.view(-1, *([1] * (stacked.ndim - 1)))).sum(dim=0)
                        vl = evaluate_weights(loss_sd)
                        if vl < best_val_loss:
                            best_val_loss = vl
                            best_label = "loss_weighted_swa"
                        del loss_sd
                except Exception as e:
                    print0(f"Loss-weighted SWA eval failed: {e}")

                # --- Candidate 4: Tail SWA (last 64 snapshots only, uniform) ---
                try:
                    tail_count = min(64, n_snaps)
                    tail_sd = {}
                    for name in param_names:
                        stacked = torch.stack([s[1][name] for s in swa_snapshots[-tail_count:]])
                        tail_sd[name] = stacked.mean(dim=0)
                    vl = evaluate_weights(tail_sd)
                    if vl < best_val_loss:
                        best_val_loss = vl
                        best_label = f"tail{tail_count}_swa"
                    del tail_sd
                except Exception as e:
                    print0(f"Tail SWA eval failed: {e}")

                # --- Candidate 5: Tail SWA (last 32 snapshots, linear weighted) ---
                try:
                    tail_count = min(32, n_snaps)
                    tail_sd = {}
                    tw = torch.arange(1, tail_count + 1, dtype=torch.float32)
                    tw = tw / tw.sum()
                    for name in param_names:
                        stacked = torch.stack([s[1][name] for s in swa_snapshots[-tail_count:]])
                        tail_sd[name] = (stacked * tw.view(-1, *([1] * (stacked.ndim - 1)))).sum(dim=0)
                    vl = evaluate_weights(tail_sd)
                    if vl < best_val_loss:
                        best_val_loss = vl
                        best_label = f"tail{tail_count}_linear_swa"
                    del tail_sd
                except Exception as e:
                    print0(f"Tail linear SWA eval failed: {e}")

                # --- Candidate 6-8: EMA shadows ---
                if ema_initialized:
                    for i, decay in enumerate(EMA_DECAYS):
                        try:
                            vl = evaluate_weights(ema_shadows[i])
                            if vl < best_val_loss:
                                best_val_loss = vl
                                best_label = f"ema_{decay}"
                        except Exception as e:
                            print0(f"EMA {decay} eval failed: {e}")
        ...
        # =========================================================================
        # Post-step: Update EMA and collect SWA snapshots (NO training modification)
        # =========================================================================
        ...
        if not ema_initialized:
            for decay in EMA_DECAYS:
                shadow = {}
                for name, param in raw_model.named_parameters():
                    shadow[name] = param.data.detach().cpu().clone()
        ...
        # All-reduce train_loss for logging (and for loss-weighted SWA)
        ...
        if step >= SWA_START_STEP and step % SWA_FREQ == 0:
            ...
            swa_snapshots.append((step, snapshot))
            # Record the training loss for this step for loss-weighted averaging
            swa_losses.append(train_loss.item())
