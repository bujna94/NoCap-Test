...
class Rotary(torch.nn.Module):
    def __init__(self, dim, max_seq_len=1024, base=10000):
        ...
        self.register_buffer("cos_cached", freqs.cos().clone())
        self.register_buffer("sin_cached", freqs.sin().clone())
...
class GPT(nn.Module):
    ...
    def configure_optimizers(self, weight_decay, learning_rate, betas, device_type, fused=True):
...
if __name__ == "__main__":
    ...
    print0(f"Using micro-batch size: {B}, grad_accum: {args.grad_accumulation_steps}")
    ...
    def get_lr(it):
        ...
        if it < WARMUP_OVERRIDE:
            return PEAK_LR * (it + 1) / WARMUP_OVERRIDE
        elif it < args.num_iterations - WARMDOWN_OVERRIDE:
            return PEAK_LR
        else:
            decay_ratio = (args.num_iterations - it) / WARMDOWN_OVERRIDE
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            if last_step:
                # Swap in EMA weights for final evaluation only
                raw_params_backup = []
                for p, ep in zip(raw_model.parameters(), ema_params):
                ...
                # Restore raw weights (for checkpoint saving if needed)
                for p, rp in zip(raw_model.parameters(), raw_params_backup):
                    p.data.copy_(rp)

                print0(f"step:{step}/{args.num_iterations} | val loss (ema):{val_loss:.6f}")
            else:
                with torch.no_grad():
                    val_loss = 0.0
                    for _ in range(current_val_steps):
            ...
            if master_process and logfile is not None:
                with open(logfile, "a") as f:
                    f.write("s:%d val:%f\n" % (step, val_loss.item() if hasattr(val_loss, 'item') else val_loss))
        ...
        if step % LOG_EVERY == 0 or step < 2:
            ...
            if master_process and logfile is not None:
                with open(logfile, "a") as f:
                    f.write("s:%d trn:%f\n" % (step, loss.item() * args.grad_accumulation_steps))
