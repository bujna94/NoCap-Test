...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Learnable logit scale - allows decoupling embedding norm from output scale
    ...
    def forward(self, idx, targets=None, return_logits=True):
        ...
        if targets is not None:
            ...
            logits = self.lm_head(
                x[:, [-1], :]
            ) * self.logit_scale  # note: using list [-1] to preserve the time dim
            loss = None
...
if __name__ == "__main__":
    ...
    # ---- SWA + EMA setup ----
    # Dense SWA: collect snapshots every 8 steps over last 1024 steps
    SWA_START_STEP = args.num_iterations - 1024  # step 3744
    SWA_EVERY = 8
    swa_snapshots = []
    
    # Multiple EMA shadows with different decays
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            # Collect candidates for evaluation
            candidates = []
            candidate_names = []
            
            # Candidate 0: current model weights (baseline)
            candidates.append(None)  # None means use current weights
            candidate_names.append("current")
            
            # Candidate 1: EMA weights for each decay
            for i, decay in enumerate(EMA_DECAYS):
                candidates.append(ema_shadows[i])
                candidate_names.append(f"ema_{decay}")
            
            # Candidate 2: SWA average (if we have snapshots)
            if len(swa_snapshots) >= 2:
                swa_avg = {}
                n_snaps = len(swa_snapshots)
                for name in swa_snapshots[0]:
                    swa_avg[name] = sum(s[name] for s in swa_snapshots) / n_snaps
                candidates.append(swa_avg)
                candidate_names.append(f"swa_{n_snaps}")
                
                # Candidate 3: linearly-weighted SWA (more weight to later snapshots)
                lin_swa_avg = {}
                weights = torch.arange(1, n_snaps + 1, dtype=torch.float32)
                weights = weights / weights.sum()
                for name in swa_snapshots[0]:
                    lin_swa_avg[name] = sum(w.item() * s[name] for w, s in zip(weights, swa_snapshots))
                candidates.append(lin_swa_avg)
                candidate_names.append(f"lin_swa_{n_snaps}")
            ...
            for cand_idx, cand in enumerate(candidates):
                ...
                if val_loss_val < best_val_loss:
                    ...
                    best_name = candidate_names[cand_idx]
                
                # Restore original weights after each candidate
                if cand is not None:
                    for name, param in raw_model.named_parameters():
                        param.data.copy_(original_state[name])
            
            # log to console and to file
            print0(f"step:{step}/{args.num_iterations} | val loss {best_val_loss:.6f} | best_candidate: {best_name}")
            if master_process:
                if args.log_wandb:
