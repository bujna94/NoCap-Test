...
if __name__ == "__main__":
    ...
    # =========================================================================
    # Weight averaging setup - training is completely unchanged
    # =========================================================================
    # EMA shadows with multiple decay rates
    ema_decays = [0.997, 0.998, 0.999, 0.9993, 0.9995]
    ema_shadows = []
    for _ in ema_decays:
        shadow = {}
    ...
    # SWA snapshot collection config
    # Wide window: every 8 steps over last 1024 steps
    swa_wide_start = args.num_iterations - 1024
    swa_wide_interval = 8
    swa_wide_snapshots = []
    swa_wide_count = 0

    # Dense window: every 2 steps over last 256 steps
    swa_dense_start = args.num_iterations - 256
    swa_dense_interval = 2
    swa_dense_snapshots = []
    swa_dense_count = 0

    # Very dense: every step over last 64 steps
    swa_vdense_start = args.num_iterations - 64
    swa_vdense_snapshots = []
    swa_vdense_count = 0

    def collect_snapshot(snapshot_list, count_ref):
        """Collect a running average snapshot (memory efficient)"""
        if len(snapshot_list) == 0:
            # First snapshot: clone all params
            snap = {}
            for name, param in raw_model.named_parameters():
            ...
            snap = snapshot_list[0]
            n = count_ref + 1
            for name, param in raw_model.named_parameters():
                snap[name].lerp_(param.data, 1.0 / n)
            return n

    def collect_snapshot_weighted(snapshot_list, weight, total_weight_list):
        """Collect a weighted running average snapshot"""
        if len(snapshot_list) == 0:
            snap = {}
            for name, param in raw_model.named_parameters():
                snap[name] = param.data.clone() * weight
            snapshot_list.append(snap)
            total_weight_list[0] = weight
        else:
            snap = snapshot_list[0]
            for name, param in raw_model.named_parameters():
                snap[name].add_(param.data * weight)
            total_weight_list[0] += weight

    # Linear-weighted SWA over last 1024 steps
    swa_linear_snapshots = []
    swa_linear_total_weight = [0.0]
    swa_linear_start = args.num_iterations - 1024
    swa_linear_interval = 8

    # Quadratic-weighted SWA over last 1024 steps
    swa_quad_snapshots = []
    swa_quad_total_weight = [0.0]
    swa_quad_start = args.num_iterations - 1024
    swa_quad_interval = 8
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            # Now try each candidate averaged weight set
            ...
            # SWA uniform wide
            if len(swa_wide_snapshots) > 0 and swa_wide_count > 0:
                candidates.append(("swa_wide_uniform", swa_wide_snapshots[0]))

            # SWA uniform dense
            if len(swa_dense_snapshots) > 0 and swa_dense_count > 0:
                candidates.append(("swa_dense_uniform", swa_dense_snapshots[0]))

            # SWA very dense
            if len(swa_vdense_snapshots) > 0 and swa_vdense_count > 0:
                candidates.append(("swa_vdense_uniform", swa_vdense_snapshots[0]))

            # SWA linear-weighted
            if len(swa_linear_snapshots) > 0 and swa_linear_total_weight[0] > 0:
                # Need to finalize by dividing by total weight
                finalized_linear = {}
                tw = swa_linear_total_weight[0]
                for name in swa_linear_snapshots[0]:
                    finalized_linear[name] = swa_linear_snapshots[0][name] / tw
                candidates.append(("swa_linear", finalized_linear))

            # SWA quadratic-weighted
            if len(swa_quad_snapshots) > 0 and swa_quad_total_weight[0] > 0:
                finalized_quad = {}
                tw = swa_quad_total_weight[0]
                for name in swa_quad_snapshots[0]:
                    finalized_quad[name] = swa_quad_snapshots[0][name] / tw
                candidates.append(("swa_quad", finalized_quad))
            ...
            if len(candidates) > 0:
                ...
                for cand_name, cand_weights in candidates:
                    # Swap in candidate weights
                    for name, param in raw_model.named_parameters():
                        param.data.copy_(cand_weights[name])
            ...
            print0(f"step:{step}/{args.num_iterations} | val loss {val_loss_report:.6f} (best: {best_source}, current: {current_val_loss:.6f})")
            if master_process:
                if args.log_wandb:
        ...
        for i, decay in enumerate(ema_decays):
            shadow = ema_shadows[i]
            for name, param in raw_model.named_parameters():
                shadow[name].lerp_(param.data, 1.0 - decay)

        # Collect SWA snapshots
        # Wide uniform SWA
        if step >= swa_wide_start and (step - swa_wide_start) % swa_wide_interval == 0:
            swa_wide_count = collect_snapshot(swa_wide_snapshots, swa_wide_count)

        # Dense uniform SWA
        if step >= swa_dense_start and (step - swa_dense_start) % swa_dense_interval == 0:
            swa_dense_count = collect_snapshot(swa_dense_snapshots, swa_dense_count)

        # Very dense uniform SWA
        if step >= swa_vdense_start:
            swa_vdense_count = collect_snapshot(swa_vdense_snapshots, swa_vdense_count)

        # Linear-weighted SWA
        if step >= swa_linear_start and (step - swa_linear_start) % swa_linear_interval == 0:
            weight = float(step - swa_linear_start + 1)
            collect_snapshot_weighted(swa_linear_snapshots, weight, swa_linear_total_weight)

        # Quadratic-weighted SWA
        if step >= swa_quad_start and (step - swa_quad_start) % swa_quad_interval == 0:
            weight = float((step - swa_quad_start + 1) ** 2)
            collect_snapshot_weighted(swa_quad_snapshots, weight, swa_quad_total_weight)
