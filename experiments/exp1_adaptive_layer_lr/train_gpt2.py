...
# -----------------------------------------------------------------------------
# Adaptive Learning Rate Optimizer Wrapper

class AdaptiveLayerLROptimizer:
    def __init__(self, base_optimizer, model, smoothing=0.9, min_scale=0.1, max_scale=3.0):
        ...
        # Initialize running statistics for each parameter group
        self.grad_mean_ema = {}
        self.grad_std_ema = {}
        
        # Group parameters by layer
        self.layer_param_groups = self._group_params_by_layer()
        
    def _group_params_by_layer(self):
        """Group parameters by their layer/module for per-layer LR scaling"""
        layer_groups = {}
        for name, param in self.model.named_parameters():
            if param.requires_grad:
                # Extract layer identifier from parameter name
                if 'transformer.h.' in name:
                    layer_id = name.split('transformer.h.')[1].split('.')[0]
                ...
                layer_groups[layer_key].append((name, param))
        
        return layer_groups
    
    def _compute_layer_lr_scales(self):
        """Compute per-layer learning rate scales based on gradient statistics"""
        layer_scales = {}
        
        for layer_key, params in self.layer_param_groups.items():
            grad_values = []
            
            for name, param in params:
                if param.grad is not None:
                    grad_values.append(param.grad.detach().flatten())
            
            if len(grad_values) == 0:
                layer_scales[layer_key] = 1.0
                continue
                
            # Concatenate all gradients for this layer
            layer_grads = torch.cat(grad_values)
            
            # Compute statistics
            grad_mean = layer_grads.abs().mean().item()
            grad_std = layer_grads.std().item() + 1e-8
            
            # Initialize EMA if needed
            if layer_key not in self.grad_mean_ema:
                self.grad_mean_ema[layer_key] = grad_mean
                self.grad_std_ema[layer_key] = grad_std
            else:
                # Update EMA
                self.grad_mean_ema[layer_key] = (
                    self.smoothing * self.grad_mean_ema[layer_key] + 
                    (1 - self.smoothing) * grad_mean
                )
                self.grad_std_ema[layer_key] = (
                    self.smoothing * self.grad_std_ema[layer_key] + 
                    (1 - self.smoothing) * grad_std
                )
            
            # Compute signal-to-noise ratio
            snr = self.grad_mean_ema[layer_key] / (self.grad_std_ema[layer_key] + 1e-8)
            
            # Scale learning rate based on SNR (higher SNR = more stable = can use higher LR)
            # Use sigmoid-like scaling to keep it bounded
            scale = 1.0 + (self.max_scale - 1.0) * torch.sigmoid(torch.tensor(snr - 1.0)).item()
            scale = max(self.min_scale, min(self.max_scale, scale))
            
            layer_scales[layer_key] = scale
        
        return layer_scales
    
    def step(self):
        # Compute per-layer learning rate scales
        layer_scales = self._compute_layer_lr_scales()
        
        # Store original learning rates
        original_lrs = [group['lr'] for group in self.base_optimizer.param_groups]
        
        # Apply layer-specific scaling
        param_to_layer = {}
        for layer_key, params in self.layer_param_groups.items():
            for name, param in params:
                param_to_layer[id(param)] = layer_scales[layer_key]
        
        # Scale learning rates in optimizer param groups
        for group in self.base_optimizer.param_groups:
            for param in group['params']:
                if id(param) in param_to_layer:
                    group['lr'] = original_lrs[0] * param_to_layer[id(param)]
                    break
        
        # Perform the optimization step
        self.base_optimizer.step()
        
        # Restore original learning rates for next iteration
...
if __name__ == "__main__":
    ...
    # init the optimizer
    base_optimizer = raw_model.configure_optimizers(
    ...
    # Wrap with adaptive layer LR optimizer
    optimizer = AdaptiveLayerLROptimizer(base_optimizer, raw_model)
