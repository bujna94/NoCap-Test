...
if __name__ == "__main__":
    ...
    # ---- EMA setup ----
    # We maintain EMA shadow weights for evaluation
    # EMA decay parameter - higher = smoother averaging
    EMA_DECAY = 0.998
    # Start EMA after this many steps (let model train a bit first)
    EMA_START_STEP = 2000
    # Store EMA weights as a flat list of tensors (simple, no dict overhead)
    ema_params = None  # will be initialized at EMA_START_STEP
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            # If we have EMA params, swap them in for evaluation
            use_ema = (ema_params is not None)
            if use_ema:
                # Save current training params and load EMA params
                saved_params = []
            ...
            # Restore training params if we swapped
            if use_ema:
        ...
        # ---- EMA update ----
        # Initialize EMA params at EMA_START_STEP
        if step == EMA_START_STEP:
            ema_params = [p.data.clone() for p in raw_model.parameters()]
        # Update EMA params after EMA_START_STEP
