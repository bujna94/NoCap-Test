...
if __name__ == "__main__":
    ...
    total_grad_accum = args.grad_accumulation_steps
    assert total_grad_accum % ddp_world_size == 0
    per_gpu_grad_accum_original = total_grad_accum // ddp_world_size
    tokens_per_step_per_gpu = B_original * per_gpu_grad_accum_original * T

    B = MICRO_BATCH
    new_grad_accum = max(1, tokens_per_step_per_gpu // (B * T))
    actual_tokens_per_step_per_gpu = B * new_grad_accum * T
    print0(f"B={B}, new_grad_accum={new_grad_accum}, tokens/step/gpu={actual_tokens_per_step_per_gpu} (target={tokens_per_step_per_gpu})")
    ...
    print0(f"Using B={B}, grad_accum={grad_accumulation_steps}, total_tokens/step={tokens_per_iter}")
    ...
    for step in range(num_iterations + 1):
        ...
        if val_loss_every > 0 and (step % val_loss_every == 0 or last_step):
            ...
            if last_step:
                ...
                original_params = []
                for ema_p, model_p in zip(ema_params, raw_model.parameters()):
                    original_params.append(model_p.data)
                    model_p.data = ema_p
                ...
                for orig_p, model_p in zip(original_params, raw_model.parameters()):
                    model_p.data = orig_p

                val_loss = min(val_loss_raw, val_loss_ema)
                print0(f"step:{step}/{num_iterations} | val loss raw={val_loss_raw:.6f} ema={val_loss_ema:.6f} best={val_loss:.6f}")
            else:
                with torch.no_grad():
                    val_loss = 0.0
                    for _ in range(val_steps):
        ...
        if step >= warmup_iters:
            in_warmdown = (step >= warmdown_start)
            if in_warmdown:
