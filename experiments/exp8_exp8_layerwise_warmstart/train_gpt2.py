...
if __name__ == "__main__":
    ...
    # NEW: warmstart freezing parameter
    parser.add_argument(
        "--warmstart_freeze_steps",
        type=int,
        default=1400,
        help="number of steps to freeze transformer blocks during warmstart",
    )
    ...
    # NEW: Freeze transformer blocks during warmstart phase
    def freeze_transformer_blocks(model):
        """Freeze all transformer block parameters (attention and MLP)"""
        for block in model.transformer.h:
            for param in block.parameters():
                param.requires_grad = False
        print0("Frozen transformer blocks for warmstart phase")
    
    def unfreeze_transformer_blocks(model):
        """Unfreeze all transformer block parameters"""
        for block in model.transformer.h:
            for param in block.parameters():
                param.requires_grad = True
        print0("Unfrozen transformer blocks - full training enabled")
    
    # Start with frozen blocks
    freeze_transformer_blocks(model)
    ...
    for step in range(args.num_iterations + 1):
        ...
        # NEW: Unfreeze transformer blocks after warmstart phase
        if step == args.warmstart_freeze_steps:
            unfreeze_transformer_blocks(raw_model)
