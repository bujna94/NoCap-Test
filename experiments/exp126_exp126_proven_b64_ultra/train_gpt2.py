...
if __name__ == "__main__":
    ...
    grad_accum_total = max(1, orig_tokens_per_step // (B * T))
    assert grad_accum_total % ddp_world_size == 0, f"grad_accum_total {grad_accum_total} must be divisible by world_size {ddp_world_size}"
    grad_accum_steps = grad_accum_total // ddp_world_size
    ...
    print0(f"micro batch size: {B}, grad_accum_steps per GPU: {grad_accum_steps}")
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            is_final = last_step
            current_val_batches = val_steps if is_final else INTERMEDIATE_VAL_BATCHES
            ...
            if is_final:
                ...
                orig_data = []
                for p, ep in zip(model_params, ema_params):
                    orig_data.append(p.data)
                    p.data = ep
                ...
                for p, od in zip(model_params, orig_data):
                    p.data = od
        ...
        with torch.no_grad():
            for ep, p in zip(ema_params, raw_model.parameters()):
                ep.lerp_(p.data, 1.0 - EMA_DECAY)
