...
TUNED_WARMDOWN = 512
EMA_DECAY = 0.998
INTERMEDIATE_VAL_BATCHES = 8
MICRO_BS = 8  # larger micro-batch for fewer kernel launches
GRAD_ACCUM = 16  # total effective batch same as B=4*accum=32 -> 128 seqs
...
if __name__ == "__main__":
    ...
    parser.add_argument("--batch_size", type=int, default=MICRO_BS)
    parser.add_argument("--grad_accumulation_steps", type=int, default=GRAD_ACCUM)
    ...
    print0(f"micro batch size: {B}, grad_accum: {args.grad_accumulation_steps}")
    ...
    print0("compiling the model with reduce-overhead mode...")
    try:
        model = torch.compile(model, mode="reduce-overhead")
        print0("Successfully set reduce-overhead mode")
    except Exception as e:
        print0(f"reduce-overhead failed ({e}), falling back to default compile")
