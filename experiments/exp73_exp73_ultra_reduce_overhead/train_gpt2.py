...
if __name__ == "__main__":
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            if last_step:
                ...
                current_val_steps = EXP_REDUCED_VAL_BATCHES
        ...
        # Sparse logging: only sync and log every EXP_LOG_EVERY steps to avoid CPU-GPU stalls
        if step % EXP_LOG_EVERY == 0 or step < 5:
            torch.cuda.synchronize()
    ...
    # Final timing sync
    torch.cuda.synchronize()
    total_training_time_ms = training_time_ms + 1000 * (time.perf_counter() - t0)
    print0(f"total training time: {total_training_time_ms/1000:.2f}s")
    print0(
