...
# ------------- TUNED HYPERPARAMETERS (hardcoded) -------------
TUNED_LR = 0.003
TUNED_WARMUP = 256
TUNED_WARMDOWN = 1024
EMA_DECAY = 0.998
...
if __name__ == "__main__":
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (last_step or (step > 0 and step % args.val_loss_every == 0)):
            torch.cuda.synchronize()
            ...
            if last_step:
                # Full validation with EMA weights at final step
                orig_params = []
                ...
                # Intermediate: use only INTERMEDIATE_VAL_BATCHES for speed
                with torch.no_grad():
                    val_loss = 0.0
                    for _ in range(INTERMEDIATE_VAL_BATCHES):
                        x_val, y_val = val_loader.next_batch()
                        _, loss = model(x_val, y_val, return_logits=False)
                        val_loss += loss
                    dist.all_reduce(val_loss, op=dist.ReduceOp.AVG)
                    val_loss /= INTERMEDIATE_VAL_BATCHES

                print0(f"step:{step}/{args.num_iterations} | val loss {val_loss:.6f} (approx, {INTERMEDIATE_VAL_BATCHES} batches)")
                if master_process and logfile is not None:
                    with open(logfile, "a") as f:
                        f.write("s:%d val:%f\n" % (step, val_loss.item()))
