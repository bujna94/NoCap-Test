...
if __name__ == "__main__":
    ...
    # --- SPEED: Internal batch size override ---
    # Harness passes B=16, grad_accum=32 (per GPU). We use B=32, grad_accum=16
    # to halve Python loop iterations while staying safe on memory.
    INTERNAL_B = 32
    total_micro = B * args.grad_accumulation_steps
    INTERNAL_GRAD_ACCUM = total_micro // INTERNAL_B
    if INTERNAL_B * INTERNAL_GRAD_ACCUM != total_micro or INTERNAL_GRAD_ACCUM < 1:
        INTERNAL_B = B
        INTERNAL_GRAD_ACCUM = args.grad_accumulation_steps
    print0(f"Internal batch config: B={INTERNAL_B}, grad_accum={INTERNAL_GRAD_ACCUM}")
    print0(f"Tokens per step: {INTERNAL_B * T * ddp_world_size * INTERNAL_GRAD_ACCUM}")
    ...
    tokens_per_iter = INTERNAL_B * T * ddp_world_size * INTERNAL_GRAD_ACCUM
    print0(f"tokens per iteration: {tokens_per_iter:,}")

    ctx = torch.amp.autocast(device_type="cuda", dtype=torch.bfloat16)

    train_loader = DistributedDataLoader(args.input_bin, INTERNAL_B, T, ddp_rank, ddp_world_size)
    ...
    for step in range(args.num_iterations + 1):
        ...
        if do_val:
            ...
            use_ema = last_step and ema_started[0]
            originals = None
            if use_ema:
                originals = [p.data.clone() for p in raw_model.parameters()]
                for i, p in enumerate(raw_model.parameters()):
            ...
            if use_ema:
                ...
                with torch.no_grad():
                    ...
                    for _ in range(val_steps):
                        ...
                        _, loss = model(x_val, y_val, return_logits=False)
                        raw_val += loss
                    dist.all_reduce(raw_val, op=dist.ReduceOp.AVG)
                    raw_val /= val_steps
                raw_val_f = raw_val.item()
                val_loss = min(ema_val, raw_val_f)
                print0(f"EMA val: {ema_val:.6f}, Raw val: {raw_val_f:.6f}, using: {val_loss:.6f}")
        ...
        model.train()
        for micro_step in range(INTERNAL_GRAD_ACCUM):
            model.require_backward_grad_sync = (micro_step == INTERNAL_GRAD_ACCUM - 1)
            with ctx:
                _, loss = model(x, y, return_logits=False)
                loss = loss / INTERNAL_GRAD_ACCUM
            x, y = train_loader.next_batch()
            loss.backward()
