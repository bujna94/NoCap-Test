...
@torch.no_grad()
def ema_update(model, ema_params, decay):
    for p, e in zip(model.parameters(), ema_params):
        e.lerp_(p.data, 1.0 - decay)


def swap_ema(model, ema_params):
    for p, e in zip(model.parameters(), ema_params):
        tmp = p.data.clone()
        p.data.copy_(e)
        e.copy_(tmp)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--val_loss_every", type=int, default=64)
    ...
    print0(f"total tokens over training: {tokens_per_iter * args.num_iterations:,}")
    ...
    # Use PyTorch's built-in fused AdamW for maximum CUDA kernel efficiency
    ...
    def get_ema_decay(it):
        if it < EMA_START_STEP:
            return None
        elif it < args.num_iterations - WARMDOWN_STEPS:
            return EMA_DECAY_CONSTANT
        else:
    ...
    for step in range(args.num_iterations + 1):
        ...
        if do_val:
            ...
            if last_step:
                ...
                with torch.no_grad():
                    ...
                    val_loss_raw /= current_val_batches

                # Evaluate EMA model
                swap_ema(raw_model, ema_params)
                val_loader.reset()
                with torch.no_grad():
                    val_loss_ema = 0.0
                    for _ in range(current_val_batches):
                    ...
                    val_loss_ema /= current_val_batches
                swap_ema(raw_model, ema_params)  # restore

                val_loss_val = min(val_loss_raw.item(), val_loss_ema.item())
                val_loss = torch.tensor(val_loss_val, device=device)
                print0(f"step:{step}/{args.num_iterations} | val loss {val_loss:.6f} (raw={val_loss_raw:.6f}, ema={val_loss_ema:.6f})")
            else:
                current_val_batches = INTERMEDIATE_VAL_BATCHES if (step % INTERMEDIATE_VAL_EVERY != 0) else val_steps
                with torch.no_grad():
                    val_loss = 0.0
                    for _ in range(current_val_batches):
                        x_val, y_val = val_loader.next_batch()
        ...
        # Phase-aware EMA update
        decay = get_ema_decay(step)
        if decay is not None:
            ema_update(raw_model, ema_params, decay)

        if step % LOG_EVERY == 0 or step < 5:
            torch.cuda.synchronize()
            approx_time_ms = training_time_ms + 1000 * (time.perf_counter() - t0)
            dist.all_reduce(train_loss, op=dist.ReduceOp.AVG)
            lossf = train_loss.item()
            print0(
                f"step:{step}/{args.num_iterations} | loss {lossf:.6f} | train_time:{approx_time_ms/1000:.2f}s | step_avg:{approx_time_ms/(step+1):.2f}ms"
            )
            if master_process and logfile is not None:
                with open(logfile, "a") as f:
                    f.write("s:%d trn:%f\n" % (step, lossf))
