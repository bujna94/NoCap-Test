...
if __name__ == "__main__":
    ...
    print0(f"batch_size: {B}, grad_accum: {args.grad_accumulation_steps}")
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            if is_final:
                ...
                for p, raw_p in zip(raw_model.parameters(), raw_params):
                    p.data = raw_p
                ...
                do_intermediate = (step == 0) or (step % INTERMEDIATE_VAL_EVERY == 0)
                if do_intermediate:
                    actual_val_batches = INTERMEDIATE_VAL_BATCHES
                    with torch.no_grad():
                        val_loss = 0.0
                        for _ in range(actual_val_batches):
                            x_val, y_val = val_loader.next_batch()
                            _, loss = model(x_val, y_val, return_logits=False)
                            val_loss += loss
                        dist.all_reduce(val_loss, op=dist.ReduceOp.AVG)
                        val_loss /= actual_val_batches
                    print0(f"step:{step}/{args.num_iterations} | val loss {val_loss:.6f} (approx)")
                else:
                    val_loss = float('nan')
                    print0(f"step:{step}/{args.num_iterations} | val skip")
