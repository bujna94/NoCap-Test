...
if __name__ == "__main__":
    ...
    # ---- EMA setup ----
    # Adaptive EMA: decay increases linearly from ema_decay_start to ema_decay_end over training
    # This avoids over-averaging early weights while smoothing final oscillations
    EMA_DECAY_START = 0.99
    EMA_DECAY_END = 0.9999
    EMA_START_STEP = 0  # start EMA from the beginning (exp29 showed this is better than late start)
    
    # Create EMA shadow parameters as plain tensors (not nn.Parameters)
    ema_params = {}
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            if step >= EMA_START_STEP:
                # Save current weights and load EMA weights
                saved_params = {}
                for name, param in raw_model.named_parameters():
        ...
        # Update EMA parameters with adaptive decay
        if step >= EMA_START_STEP:
            # Linear interpolation of decay from EMA_DECAY_START to EMA_DECAY_END
            progress = min(1.0, step / max(1, args.num_iterations - 1))
