...
if __name__ == "__main__":
    ...
    B_orig, T = args.batch_size, args.sequence_length
    ...
    orig_grad_accum = args.grad_accumulation_steps // ddp_world_size
    total_micro_per_step = B_orig * orig_grad_accum

    MICRO_BATCH = 64
    grad_accum_steps = total_micro_per_step // MICRO_BATCH
    if grad_accum_steps < 1:
        grad_accum_steps = 1
        MICRO_BATCH = total_micro_per_step
    B = MICRO_BATCH

    tokens_per_iter = B * T * ddp_world_size * grad_accum_steps
    print0(f"tokens per iteration: {tokens_per_iter:,} (B={B}, grad_accum={grad_accum_steps})")
    ...
    for step in range(args.num_iterations + 1):
        ...
        if step % LOG_EVERY == 0 or step < 2:
            ...
            print0(f"step:{step}/{args.num_iterations} | lr {lr:.6f} | train_time:{approx_time_ms/1000:.2f}s | step_avg:{approx_time_ms/(step+1):.2f}ms")
            if master_process and logfile is not None:
                with open(logfile, "a") as f:
                    f.write("s:%d trn:%f\n" % (step, 0.0))
