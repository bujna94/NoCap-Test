...
if __name__ == "__main__":
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            if last_step:
                # Full validation with both raw and EMA
                with torch.no_grad():
                    val_loss_raw = 0.0
                ...
                print0(f"step:{step}/{args.num_iterations} val_loss_raw:{val_loss_raw:.6f} val_loss_ema:{val_loss_ema:.6f} val_loss:{val_loss:.6f} train_time:{training_time_ms:.0f}ms")
            else:
                ...
                print0(f"step:{step}/{args.num_iterations} val_loss:{val_loss:.6f} train_time:{training_time_ms:.0f}ms")
        ...
        # Sparse logging without sync
        if master_process and (step % LOG_EVERY == 0 or step < 3):
            approx_time = training_time_ms + 1000 * (time.perf_counter() - t0)
            print0(f"step:{step}/{args.num_iterations} train_loss:{train_loss.item():.4f} lr:{lr:.6f} step_avg:{approx_time/(step+1):.2f}ms")
