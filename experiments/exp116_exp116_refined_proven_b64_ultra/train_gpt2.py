...
if __name__ == "__main__":
    ...
    effective_grad_accum = ACTUAL_GRAD_ACCUM
    ...
    print0(f"micro_batch={B}, grad_accum_per_gpu={grad_accum_per_gpu}, seq_len={T}")
    ...
    def get_lr(it):
        ...
        if it < ACTUAL_WARMUP:
            return ACTUAL_LR * (it + 1) / ACTUAL_WARMUP
        elif it < args.num_iterations - ACTUAL_WARMDOWN:
            return ACTUAL_LR
        else:
            decay_ratio = (args.num_iterations - it) / ACTUAL_WARMDOWN
            return ACTUAL_LR * decay_ratio
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            if last_step:
                ...
                current_val_batches = min(INTERMEDIATE_VAL_BATCHES, val_steps)
            ...
            if last_step:
                ...
                for i, p in enumerate(raw_params):
                    p.data, ema_params[i] = ema_params[i], p.data
        ...
        if step % LOG_EVERY == 0 or step < 2:
            ...
            if master_process and logfile is not None:
                with open(logfile, "a") as f:
                    f.write("s:%d trn:%f\n" % (step, loss.item() * grad_accum_per_gpu))
