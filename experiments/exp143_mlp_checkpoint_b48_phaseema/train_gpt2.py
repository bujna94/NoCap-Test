...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        # Apply gradient checkpointing to MLP to save activation memory
        mlp_input = rmsnorm(x)
        x = x + grad_checkpoint(self.mlp, mlp_input, use_reentrant=False)
        return x
...
if __name__ == "__main__":
    ...
    # Fixed grad_accum to maintain ~540K tokens/step (close to baseline 524K)
    # B=48, T=1024, grad_accum=11 => 48*11*1024 = 540672 tokens/step (per GPU)
    # With DDP, divide by world_size
    grad_accum_steps = GRAD_ACCUM_FIXED
    if ddp_world_size > 1:
        grad_accum_steps = max(1, GRAD_ACCUM_FIXED // ddp_world_size)

    device = f"cuda:{ddp_local_rank}"
    torch.cuda.set_device(device)
    master_process = ddp_rank == 0
    print0(f"using device: {device}")
    print0(f"Micro-batch: {B}, grad_accum_steps (per GPU): {grad_accum_steps}, T: {T}")
    print0(f"Effective tokens/step: {B * grad_accum_steps * T * ddp_world_size}")

    torch.set_float32_matmul_precision('high')

    tokens_per_iter = B * T * ddp_world_size * grad_accum_steps
    ...
    def swap_ema_out(backup):
        with torch.no_grad():
            for bk, p in zip(backup, raw_model.parameters()):
                p.data.copy_(bk)
    ...
    val_loss_every = args.val_loss_every if args.val_loss_every > 0 else 0
    effective_val_every = VAL_EVERY_OVERRIDE if val_loss_every > 0 else 0
    ...
    for step in range(args.num_iterations + 1):
        ...
        if val_loss_every > 0:
            if last_step or step == 0:
                is_val_step = True
            elif effective_val_every > 0 and step % effective_val_every == 0:
                is_val_step = True
        ...
        if step >= WARMUP_STEPS:
            if step < args.num_iterations - WARMDOWN_STEPS:
                update_ema(EMA_DECAY_MAIN)
            else:
                update_ema(EMA_DECAY_WARMDOWN)
