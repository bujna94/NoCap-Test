...
if __name__ == "__main__":
    ...
    args.grad_accumulation_steps = args.grad_accumulation_steps // ddp_world_size if args.grad_accumulation_steps > 1 else 1
    grad_accumulation_steps = max(1, args.grad_accumulation_steps)

    tokens_per_iter = B * grad_accumulation_steps * T * ddp_world_size
    print0(f"tokens per iteration: {tokens_per_iter:,} (B={B}, grad_accum={grad_accumulation_steps}, T={T}, world={ddp_world_size})")
    ...
    for step in range(args.num_iterations + 1):
        ...
        if step % LOG_EVERY == 0 or step < 2:
            ...
            print0(f"step:{step}/{args.num_iterations} | train_time:{approx_training_time_ms/1000:.2f}s | step_avg:{approx_training_time_ms/(step+1):.2f}ms")
            if master_process and logfile is not None:
                with open(logfile, "a") as f:
                    f.write("s:%d trn:0\n" % step)
