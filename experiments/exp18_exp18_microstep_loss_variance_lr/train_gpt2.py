    ...
    for step in range(args.num_iterations + 1):
        ...
        for micro_step in range(args.grad_accumulation_steps):
            ...
            with ctx:
                ...
                # Store the unscaled loss for variance computation
                microstep_losses.append(loss.detach().item())
                loss = (
        ...
        # Compute coefficient of variation across microstep losses
        if len(microstep_losses) > 1:
            loss_mean = np.mean(microstep_losses)
            loss_std = np.std(microstep_losses)
            # Coefficient of variation: std/mean (avoid division by zero)
            loss_cv = loss_std / (loss_mean + 1e-8)
            
            # Map CV to learning rate multiplier
            # Low CV (consistent losses) -> higher LR multiplier (up to 2.0x)
            # High CV (inconsistent losses) -> lower LR multiplier (down to 0.5x)
            # Use a smooth transition with clipping
            # CV typically ranges from 0 to ~0.3 in practice
            # Map CV=0 to 2.0x, CV=0.3 to 0.5x
            if loss_cv < 0.05:
                lr_multiplier = 2.0
            elif loss_cv > 0.25:
                lr_multiplier = 0.5
            else:
                # Linear interpolation between 0.05 and 0.25
                lr_multiplier = 2.0 - (loss_cv - 0.05) * (1.5 / 0.20)
        ...
        print0(
            f"step:{step}/{args.num_iterations} | loss {lossf:.6f} | lr_mult:{lr_multiplier:.3f} | train_time:{approx_training_time_ms/1000:.2f}s | step_avg:{approx_training_time_ms/(step+1):.2f}ms"
        )
        # log to logile
