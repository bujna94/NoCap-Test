...
class Block(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Learnable scaling parameters for residual paths
        self.register_buffer('attn_scale_adaptive', torch.tensor(1.0))
        self.register_buffer('mlp_scale_adaptive', torch.tensor(1.0))
        # EMA tracking for gradient magnitudes
        self.register_buffer('attn_grad_ema', torch.tensor(1.0))
        self.register_buffer('mlp_grad_ema', torch.tensor(1.0))
        self.ema_decay = 0.95

    def forward(self, x):
        # Store intermediate values for gradient tracking
        attn_out = self.attn_scale * self.attn(rmsnorm(x))
        attn_residual = self.attn_scale_adaptive * attn_out
        x = x + attn_residual
        
        mlp_out = self.mlp(rmsnorm(x))
        mlp_residual = self.mlp_scale_adaptive * mlp_out
        x = x + mlp_residual
    ...
    def update_scales(self):
        """Update adaptive scales based on gradient magnitudes"""
        if hasattr(self, 'attn_residual') and self.attn_residual.grad is not None:
            attn_grad_mag = self.attn_residual.grad.abs().mean().item()
            self.attn_grad_ema = self.ema_decay * self.attn_grad_ema + (1 - self.ema_decay) * attn_grad_mag
        
        if hasattr(self, 'mlp_residual') and self.mlp_residual.grad is not None:
            mlp_grad_mag = self.mlp_residual.grad.abs().mean().item()
            self.mlp_grad_ema = self.ema_decay * self.mlp_grad_ema + (1 - self.ema_decay) * mlp_grad_mag
        
        # Rebalance scales based on relative gradient magnitudes
        total_grad = self.attn_grad_ema + self.mlp_grad_ema + 1e-8
        # Scale so that the path with higher gradients gets slightly boosted
        attn_ratio = self.attn_grad_ema / total_grad
        mlp_ratio = self.mlp_grad_ema / total_grad
        
        # Smooth update: interpolate between equal scaling (0.5) and gradient-based scaling
        # This prevents extreme rebalancing
        alpha = 0.3  # how much to trust gradient signal
        self.attn_scale_adaptive = 1.0 + alpha * (attn_ratio - 0.5)
        self.mlp_scale_adaptive = 1.0 + alpha * (mlp_ratio - 0.5)
        
        # Clamp to reasonable range
        self.attn_scale_adaptive = torch.clamp(self.attn_scale_adaptive, 0.7, 1.3)
        self.mlp_scale_adaptive = torch.clamp(self.mlp_scale_adaptive, 0.7, 1.3)
...
class GPT(nn.Module):
    ...
    def update_residual_scales(self):
        """Update all block residual scales based on gradients"""
        for block in self.transformer.h:
            block.update_scales()
...
if __name__ == "__main__":
    ...
    for step in range(args.num_iterations + 1):
        ...
        # Update residual scales based on gradient flow (every 10 steps to reduce overhead)
        if step % 10 == 0 and step > 0:
            raw_model.update_residual_scales()
