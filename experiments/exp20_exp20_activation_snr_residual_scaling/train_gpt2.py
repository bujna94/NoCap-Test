...
class Block(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Exponential moving average for SNR tracking
        self.register_buffer('attn_snr_ema', torch.tensor(1.0))
        self.register_buffer('mlp_snr_ema', torch.tensor(1.0))
        self.ema_decay = 0.95

    def forward(self, x):
        # Attention path with SNR-based scaling
        attn_input = rmsnorm(x)
        attn_output = self.attn(attn_input)
        
        # Compute SNR for attention output (mean/std ratio)
        if self.training:
            with torch.no_grad():
                attn_mean = attn_output.abs().mean()
                attn_std = attn_output.std() + 1e-8
                attn_snr = attn_mean / attn_std
                # Update EMA
                self.attn_snr_ema = self.ema_decay * self.attn_snr_ema + (1 - self.ema_decay) * attn_snr
                # Clamp scale between 0.5x and 2.0x for safety
                attn_dynamic_scale = torch.clamp(self.attn_snr_ema, 0.5, 2.0)
        else:
            attn_dynamic_scale = 1.0
        
        x = x + self.attn_scale * attn_dynamic_scale * attn_output
        
        # MLP path with SNR-based scaling
        mlp_input = rmsnorm(x)
        mlp_output = self.mlp(mlp_input)
        
        # Compute SNR for MLP output
        if self.training:
            with torch.no_grad():
                mlp_mean = mlp_output.abs().mean()
                mlp_std = mlp_output.std() + 1e-8
                mlp_snr = mlp_mean / mlp_std
                # Update EMA
                self.mlp_snr_ema = self.ema_decay * self.mlp_snr_ema + (1 - self.ema_decay) * mlp_snr
                # Clamp scale between 0.5x and 2.0x for safety
                mlp_dynamic_scale = torch.clamp(self.mlp_snr_ema, 0.5, 2.0)
        else:
            mlp_dynamic_scale = 1.0
        
        x = x + mlp_dynamic_scale * mlp_output
        return x
