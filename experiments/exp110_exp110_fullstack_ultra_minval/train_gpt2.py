...
if __name__ == "__main__":
    ...
    # Compute grad accumulation to maintain total tokens per step
    actual_B = MICRO_BATCH
    total_tokens_per_step = B * (args.grad_accumulation_steps * ddp_world_size) * T
    grad_accum_total = total_tokens_per_step // (actual_B * T)
    if grad_accum_total < 1:
        grad_accum_total = 1
    assert grad_accum_total % ddp_world_size == 0, f"grad_accum_total={grad_accum_total} not divisible by world={ddp_world_size}"
    grad_accum_per_gpu = grad_accum_total // ddp_world_size
    if grad_accum_per_gpu < 1:
        grad_accum_per_gpu = 1

    tokens_per_iter = actual_B * T * ddp_world_size * grad_accum_per_gpu
    print0(f"tokens per iteration: {tokens_per_iter:,}")
    print0(f"micro_batch={actual_B}, grad_accum_per_gpu={grad_accum_per_gpu}")

    torch.set_float32_matmul_precision('high')
    ctx = torch.amp.autocast(device_type="cuda", dtype=torch.bfloat16)

    train_loader = DistributedDataLoader(args.input_bin, actual_B, T, ddp_rank, ddp_world_size)
    ...
    for step in range(args.num_iterations + 1):
        ...
        # Only validate at step 0 and final step to minimize overhead
        do_val = (args.val_loss_every > 0) and (step == 0 or last_step)
        ...
        if do_val:
            ...
            if last_step:
                # Dual evaluation: try EMA weights first
                orig_params = []
                ...
                # Restore raw weights and evaluate
                ...
                val_loss = min(val_loss_ema.item(), val_loss_raw.item())
                print0(f"step:{step}/{args.num_iterations} | val_ema:{val_loss_ema:.6f} val_raw:{val_loss_raw:.6f} | val loss {val_loss:.6f}")
            else:
                # Step 0: quick validation with subset
                with torch.no_grad():
                    val_loss = 0.0
                    quick_steps = min(4, val_steps)
                    for _ in range(quick_steps):
                        x_val, y_val = val_loader.next_batch()
                        _, loss = model(x_val, y_val, return_logits=False)
                        val_loss += loss
                    dist.all_reduce(val_loss, op=dist.ReduceOp.AVG)
                    val_loss /= quick_steps
        ...
        if step % LOG_EVERY == 0 or step < 2:
            ...
            print0(f"step:{step}/{args.num_iterations} | train_time:{approx_time/1000:.2f}s | step_avg:{approx_time/(step+1):.2f}ms")
            if master_process and logfile is not None:
                with open(logfile, "a") as f:
                    f.write("s:%d trn:0\n" % step)
