...
class Block(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.mlp = MLP(config)
        self.base_attn_scale = 1 / math.sqrt(2 * config.n_layer)
...
class GPT(nn.Module):
    ...
    def set_all_attn_scales(self, scale):
        for block in self.transformer.h:
            block.set_attn_scale(scale)
...
if __name__ == "__main__":
    ...
    # Adaptive attention scale variables
    val_loss_history = []
    base_attn_scale = 1 / math.sqrt(2 * model_config.n_layer)
    current_scale_multiplier = 1.0
    ema_improvement_rate = 0.0
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            # Adaptive attention scale adjustment based on validation loss improvement
            val_loss_value = val_loss.item()
            if len(val_loss_history) >= 2:
                # Calculate improvement rate (negative means loss is decreasing, which is good)
                recent_improvement = val_loss_history[-1] - val_loss_value
                # Update EMA of improvement rate
                ema_improvement_rate = alpha * recent_improvement + (1 - alpha) * ema_improvement_rate
                
                # Scale multiplier based on improvement rate
                # If improving rapidly (ema_improvement_rate > 0), increase attention contribution
                # If plateauing (ema_improvement_rate ~= 0), return to baseline
                # Scale between 0.8x and 1.5x to avoid extreme values
                target_multiplier = 1.0 + 0.5 * max(0, min(1.0, ema_improvement_rate / 0.1))
                # Smooth transition to avoid sudden jumps
                current_scale_multiplier = 0.7 * current_scale_multiplier + 0.3 * target_multiplier
                current_scale_multiplier = max(0.8, min(1.5, current_scale_multiplier))
                
                # Update all blocks with the new scale
                new_scale = base_attn_scale * current_scale_multiplier
                raw_model.set_all_attn_scales(new_scale)
                
            val_loss_history.append(val_loss_value)
            
            # log to console and to file
            print0(f"step:{step}/{args.num_iterations} | val loss {val_loss:.6f} | attn_scale_mult {current_scale_multiplier:.3f}")
            if master_process:
                if args.log_wandb:
                    wandb.log({"val_loss": val_loss}, step=step * tokens_per_iter)
                    wandb.log({"time": training_time_ms}, step=step * tokens_per_iter)
                    wandb.log({"attn_scale_multiplier": current_scale_multiplier}, step=step * tokens_per_iter)
