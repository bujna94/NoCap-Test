...
VOCAB_SIZE = 50304
LOG_EVERY = 128
VAL_BATCHES_INTERMEDIATE = 1
VAL_EVERY_INTERMEDIATE = 256
...
if __name__ == "__main__":
    ...
    # Compute grad_accum: harness passes total grad_accum, we divide by world_size
    assert args.grad_accumulation_steps % ddp_world_size == 0
    base_grad_accum_per_gpu = args.grad_accumulation_steps // ddp_world_size
    # Original tokens/step = args.batch_size * T * base_grad_accum_per_gpu * ddp_world_size
    total_tokens_per_step = args.batch_size * T * base_grad_accum_per_gpu * ddp_world_size
    # New grad_accum with our micro batch
    grad_accumulation_steps = max(1, total_tokens_per_step // (B * T * ddp_world_size))
    tokens_per_iter = B * T * ddp_world_size * grad_accumulation_steps
    print0(f"tokens per iteration: {tokens_per_iter:,} (target: {total_tokens_per_step:,})")
    print0(f"micro_batch: {B}, grad_accum: {grad_accumulation_steps}, world_size: {ddp_world_size}")
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0:
            if last_step:
                ...
                do_val = True
            elif step % VAL_EVERY_INTERMEDIATE == 0:
                do_val = True
        ...
        if do_val:
            ...
            if last_step:
                ...
                val_loss_report = min(ema_val_loss, raw_val_loss)
                src = "EMA" if ema_val_loss <= raw_val_loss else "raw"
                print0(f"step:{step}/{args.num_iterations} | val loss {val_loss_report:.6f} ({src}, EMA={ema_val_loss:.6f}, raw={raw_val_loss:.6f})")
                val_loss = torch.tensor(val_loss_report)
            else:
                current_val_steps = VAL_BATCHES_INTERMEDIATE
                with torch.no_grad():
                    val_loss = 0.0
                    for _ in range(current_val_steps):
                        x_val, y_val = val_loader.next_batch()
