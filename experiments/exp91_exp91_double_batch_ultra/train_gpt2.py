...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_iterations", type=int, default=2384)
    parser.add_argument("--learning_rate", type=float, default=0.00424)
    parser.add_argument("--warmup_iters", type=int, default=128)
    parser.add_argument("--warmdown_iters", type=int, default=512)
    ...
    def should_validate(step, last_step):
        ...
        if step == 0:
            return True
        if step >= LATE_VAL_THRESHOLD:
            return step % args.val_loss_every == 0
        else:
            return step % EARLY_VAL_EVERY == 0
    ...
    for step in range(args.num_iterations + 1):
        ...
        if should_validate(step, last_step):
            torch.cuda.synchronize()
