...
if __name__ == "__main__":
    ...
    # Adjust grad_accum for DDP
    assert grad_accum % ddp_world_size == 0
    grad_accum = grad_accum // ddp_world_size

    print0(f"Using B={B}, grad_accum={grad_accum}, T={T}")
    ...
    model_config = {
        "d12": GPTConfig(vocab_size=VOCAB_PAD, n_layer=12, n_head=12, n_embd=768),
        "d24": GPTConfig(vocab_size=VOCAB_PAD, n_layer=24, n_head=16, n_embd=1024),
        "d36": GPTConfig(vocab_size=VOCAB_PAD, n_layer=36, n_head=20, n_embd=1280),
        "d48": GPTConfig(vocab_size=VOCAB_PAD, n_layer=48, n_head=25, n_embd=1600),
    }[args.model]
    ...
    def get_ema_decay(it):
        if it < args.num_iterations - WARMDOWN_STEPS:
            return EMA_DECAY_NORMAL
        else:
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0:
            if last_step:
                ...
                do_val = True
            elif step % args.val_loss_every == 0:
                do_val = True
            ...
            if last_step:
                ...
                if ema_started:
                    ...
                    with torch.no_grad():
                        ...
                        val_loss_raw /= actual_val_steps

                    val_loss = min(val_loss_ema, val_loss_raw)
                    print0(f"step:{step}/{args.num_iterations} | val loss EMA {val_loss_ema:.6f} | val loss raw {val_loss_raw:.6f} | best {val_loss:.6f}")
                else:
                    with torch.no_grad():
                        val_loss = 0.0
                        for _ in range(actual_val_steps):
                ...
                actual_val_steps_here = INTERMEDIATE_VAL_BATCHES
                with torch.no_grad():
                    val_loss = 0.0
                    for _ in range(actual_val_steps_here):
                        x_val, y_val = val_loader.next_batch()
                        _, loss = model(x_val, y_val, return_logits=False)
                        val_loss += loss
                    dist.all_reduce(val_loss, op=dist.ReduceOp.AVG)
                    val_loss /= actual_val_steps_here
                print0(f"step:{step}/{args.num_iterations} | val loss {val_loss:.6f}")
