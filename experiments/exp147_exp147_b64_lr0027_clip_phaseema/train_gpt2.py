...
if __name__ == "__main__":
    ...
    gas_per_gpu_original = args.grad_accumulation_steps // ddp_world_size
    grad_accumulation_steps = max(1, (args.batch_size * gas_per_gpu_original + B - 1) // B)
    ...
    def get_lr(it):
        ...
        if it < args.warmup_iters:
            return LR_PEAK * (it + 1) / args.warmup_iters
        if it < args.num_iterations - args.warmdown_iters:
            return LR_PEAK
        decay_ratio = (args.num_iterations - it) / args.warmdown_iters
        return LR_PEAK * decay_ratio
    ...
    for step in range(args.num_iterations + 1):
        ...
        if do_val:
            ...
            if last_step:
                ...
                if ema_initialized and ema_params is not None:
                    orig_params = []
                    ...
                    if val_loss_ema.item() < val_loss_raw.item():
                        val_loss = val_loss_ema
                        print0(f"Using EMA weights (EMA: {val_loss_ema.item():.6f}, Raw: {val_loss_raw.item():.6f})")
                    else:
                        val_loss = val_loss_raw
                        print0(f"Using raw weights (EMA: {val_loss_ema.item():.6f}, Raw: {val_loss_raw.item():.6f})")
            else:
        ...
        torch.nn.utils.clip_grad_norm_(raw_model.parameters(), GRAD_CLIP_MAX)
        ...
        if step >= EMA_START_STEP:
            if step < args.num_iterations - args.warmdown_iters:
                ema_decay = EMA_DECAY_CONST
            else:
