...
EMA_DECAY = 0.9985  # effective averaging window of ~667 steps
...
if __name__ == "__main__":
    ...
    # ---- EMA Setup ----
    # Store EMA weights as a flat list of tensors, completely separate from model
    # We use .clone().detach() to ensure no shared memory with model params
    ema_params = [p.data.clone().detach() for p in raw_model.parameters()]
    print0(f"EMA initialized with decay={EMA_DECAY}")
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            # ---- Save current model params, load EMA params for evaluation ----
            # Store backup of current training weights
            saved_params = []
            ...
            for p, saved_p in zip(raw_model.parameters(), saved_params):
        ...
        # ---- Update EMA weights ----
        # This happens AFTER optimizer.step() so we average the updated params
        with torch.no_grad():
