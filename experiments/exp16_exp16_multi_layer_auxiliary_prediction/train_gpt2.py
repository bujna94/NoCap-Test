...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Add auxiliary prediction heads at intermediate layers
        self.aux_heads = nn.ModuleDict()
        self.aux_layer_indices = [3, 7]  # layers 4 and 8 (0-indexed)
        for layer_idx in self.aux_layer_indices:
            self.aux_heads[f"layer_{layer_idx}"] = nn.Linear(config.n_embd, config.vocab_size, bias=False)
    ...
    def forward(self, idx, targets=None, return_logits=True):
        ...
        for layer_idx, block in enumerate(self.transformer.h):
            ...
            # Compute auxiliary loss at specified layers
            if targets is not None and layer_idx in self.aux_layer_indices:
                x_normed = rmsnorm(x)
                aux_logits = self.aux_heads[f"layer_{layer_idx}"](x_normed)
        ...
        if targets is not None:
            ...
            # Combine main loss with auxiliary losses (weighted lower)
            loss = main_loss
            if aux_losses:
                aux_weight = 0.2
