...
if __name__ == "__main__":
    ...
    # ---- EMA setup: single shadow with decay=0.998 (proven in exp67) ----
    EMA_DECAY = 0.998
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            if last_step:
                ...
                # Evaluate EMA (decay=0.998) via zero-copy swap
                orig_data = []
                ...
                for p, orig in zip(param_list, orig_data):
                    p.data = orig

                # Pick best
                val_loss_raw_val = val_loss_raw.item() if torch.is_tensor(val_loss_raw) else val_loss_raw
                val_loss_ema_val = val_loss_ema.item() if torch.is_tensor(val_loss_ema) else val_loss_ema
                val_loss = min(val_loss_raw_val, val_loss_ema_val)
                best_name = "raw" if val_loss == val_loss_raw_val else "ema(0.998)"
                print0(f"step:{step}/{args.num_iterations} | val loss {val_loss:.6f} (best={best_name}, raw={val_loss_raw_val:.6f}, ema={val_loss_ema_val:.6f})")
            else:
                # Intermediate validation: just use raw model weights
                with torch.no_grad():
                    val_loss = 0.0
                    for _ in range(val_steps):
                        x_val, y_val = val_loader.next_batch()
            ...
            if master_process:
                if args.log_wandb:
                    val_loss_log = val_loss.item() if torch.is_tensor(val_loss) else val_loss
                    wandb.log({"val_loss": val_loss_log}, step=step * tokens_per_iter)
        ...
        for micro_step in range(args.grad_accumulation_steps):
            ...
            # Prefetch next batch before backward to overlap CPU/GPU
            x, y = train_loader.next_batch()
            loss.backward()
