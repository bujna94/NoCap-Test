...
if __name__ == "__main__":
    ...
    grad_accum_steps = args.grad_accumulation_steps // ddp_world_size
    ...
    print0(f"B={B}, grad_accum={grad_accum_steps}, T={T}")
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            with torch.no_grad():
                val_loss_raw = 0.0
                for i in range(current_val_steps):
                ...
                if last_step:
                    ...
                    if val_loss_ema < val_loss_raw:
                        ...
                        print0(f"EMA val loss: {val_loss_ema:.6f} < raw: {val_loss_raw:.6f}, using EMA")
                    else:
                        val_loss = val_loss_raw
                        print0(f"Raw val loss: {val_loss_raw:.6f} <= EMA: {val_loss_ema:.6f}, using raw")
