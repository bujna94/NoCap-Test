...
if __name__ == "__main__":
    ...
    # Exactly replicate exp105's grad_accum logic
    original_tokens_per_step = args.batch_size * args.grad_accumulation_steps * T
    new_total_grad_accum = max(1, original_tokens_per_step // (B * T))
    if ddp_world_size > 1:
        grad_accumulation_steps = max(1, new_total_grad_accum // ddp_world_size)
    else:
        grad_accumulation_steps = new_total_grad_accum

    tokens_per_iter = B * T * ddp_world_size * grad_accumulation_steps
    print0(f"tokens per iteration: {tokens_per_iter:,}")
    print0(f"micro batch: {B}, grad_accum: {grad_accumulation_steps}, world: {ddp_world_size}")
    ...
    model_config = {
        "d12": GPTConfig(vocab_size=VOCAB_SIZE_PADDED, n_layer=12, n_head=12, n_embd=768),
        "d24": GPTConfig(vocab_size=VOCAB_SIZE_PADDED, n_layer=24, n_head=16, n_embd=1024),
        "d36": GPTConfig(vocab_size=VOCAB_SIZE_PADDED, n_layer=36, n_head=20, n_embd=1280),
        "d48": GPTConfig(vocab_size=VOCAB_SIZE_PADDED, n_layer=48, n_head=25, n_embd=1600),
    }[args.model]
