...
if __name__ == "__main__":
    ...
    # Override val_loss_every for speed - evaluate much less frequently
    args.val_loss_every = VAL_LOSS_OVERRIDE
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            if is_final:
                ...
                originals = []
                for p, ema in zip(model_params, ema_params):
        ...
        if step >= EMA_START:
            ...
            # Before EMA_START, just copy weights (cold start EMA)
            if step == EMA_START - 1:
                with torch.no_grad():
                    for p, ema in zip(raw_model.parameters(), ema_params):
                        ema.copy_(p.data)
