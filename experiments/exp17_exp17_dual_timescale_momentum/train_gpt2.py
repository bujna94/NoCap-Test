...
class GPT(nn.Module):
    ...
    def configure_optimizers(self, weight_decay, learning_rate, betas, device_type):
        # Use custom dual-timescale optimizer
        optimizer = DualTimescaleMomentumAdamW(
...
# -----------------------------------------------------------------------------
# Custom Dual Timescale Momentum Optimizer

class DualTimescaleMomentumAdamW(torch.optim.Optimizer):
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.95), eps=1e-8, weight_decay=0.0):
        defaults = dict(lr=lr, betas=betas, eps=eps, weight_decay=weight_decay)
        super(DualTimescaleMomentumAdamW, self).__init__(params, defaults)
        self.fast_beta1 = 0.5  # Fast momentum for capturing recent signals
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                grad = p.grad
                if grad.is_sparse:
                    raise RuntimeError('DualTimescaleMomentumAdamW does not support sparse gradients')
                ...
                if len(state) == 0:
                    ...
                    state['fast_exp_avg'] = torch.zeros_like(p)

                exp_avg, exp_avg_sq = state['exp_avg'], state['exp_avg_sq']
                fast_exp_avg = state['fast_exp_avg']
                state['step'] += 1
                ...
                # Update biased first moment estimate (slow)
                exp_avg.mul_(beta1).add_(grad, alpha=1 - beta1)
                
                # Update biased first moment estimate (fast)
                fast_exp_avg.mul_(self.fast_beta1).add_(grad, alpha=1 - self.fast_beta1)
                ...
                fast_bias_correction1 = 1 - self.fast_beta1 ** state['step']

                # Corrected momentum estimates
                corrected_exp_avg = exp_avg / bias_correction1
                corrected_fast_exp_avg = fast_exp_avg / fast_bias_correction1

                # Compute cosine similarity between fast and slow momentum
                # Only start blending after warmup (step > 10)
                if state['step'] > 10:
                    slow_norm = corrected_exp_avg.norm()
                    fast_norm = corrected_fast_exp_avg.norm()
                    if slow_norm > 1e-8 and fast_norm > 1e-8:
                        cos_sim = (corrected_exp_avg * corrected_fast_exp_avg).sum() / (slow_norm * fast_norm + 1e-8)
                        # When momentums align (cos_sim > 0), blend toward fast momentum
                        # When they diverge (cos_sim < 0), stay with slow momentum
                        blend_weight = max(0.0, cos_sim.item())  # Range [0, 1]
                        blended_momentum = (1 - 0.3 * blend_weight) * corrected_exp_avg + 0.3 * blend_weight * corrected_fast_exp_avg
                    else:
                        blended_momentum = corrected_exp_avg
                else:
                    blended_momentum = corrected_exp_avg

                # Compute step size
                step_size = lr / bias_correction2
                denom = exp_avg_sq.sqrt().add_(eps)

                # Update parameters
                p.addcdiv_(blended_momentum, denom, value=-step_size)
