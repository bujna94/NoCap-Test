...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Adaptive sparsity: track position interaction importance
        # We'll use a simple heuristic: keep causal mask but sample subset of keys
        self.sparsity_ratio = 0.5  # only compute 50% of attention pairs
        self.step_counter = 0
    ...
    def forward(self, x):
        ...
        # Adaptive sparse attention: only use subset of keys based on position
        # Strategy: keep local context (nearby tokens) and sample distant ones
        if self.training and T > 8:
            # For each query position, we keep:
            # 1. All tokens within local_window
            # 2. Sample remaining based on distance decay
            local_window = max(8, int(T * 0.3))  # at least 8 tokens or 30% of sequence
            num_keys_to_keep = max(local_window, int(T * self.sparsity_ratio))
            
            # Create attention mask that keeps local + samples distant
            q_t = q.transpose(1, 2)  # (B, n_head, T, head_dim)
            k_t = k.transpose(1, 2)
            v_t = v.transpose(1, 2)
            
            # Compute which keys to keep for each query
            # Use position-based sampling: keep local, sample rest with decay
            positions = torch.arange(T, device=x.device)
            pos_diff = positions.unsqueeze(0) - positions.unsqueeze(1)  # (T, T)
            
            # Create probability mask: high prob for local, decay for distant
            # For causal attention, only look at past (pos_diff <= 0)
            causal_mask = (pos_diff <= 0).float()
            local_mask = (pos_diff.abs() <= local_window).float()
            
            # Probability of keeping: 1.0 for local, decaying for distant
            keep_prob = local_mask + (1 - local_mask) * (self.sparsity_ratio * 0.5)
            keep_prob = keep_prob * causal_mask  # apply causal constraint
            
            # Sample which connections to keep
            # To make it deterministic per step, use step counter as seed
            if self.step_counter % 10 == 0:  # resample every 10 steps
                random_vals = torch.rand(T, T, device=x.device)
                sparse_mask = (random_vals < keep_prob).float()
                # Ensure each query has at least some keys
                for i in range(T):
                    if sparse_mask[i, :i+1].sum() == 0 and i > 0:
                        sparse_mask[i, i] = 1.0  # at least attend to self
                self.cached_sparse_mask = sparse_mask
            
            # Use cached mask
            sparse_mask = self.cached_sparse_mask if hasattr(self, 'cached_sparse_mask') else causal_mask
            
            # Apply sparse attention using the mask
            # Convert mask to additive form for scaled_dot_product_attention
            attn_mask = torch.zeros(T, T, device=x.device)
            attn_mask = attn_mask.masked_fill(sparse_mask == 0, float('-inf'))
            ...
            # Standard full attention for eval or very short sequences
            y = F.scaled_dot_product_attention(
