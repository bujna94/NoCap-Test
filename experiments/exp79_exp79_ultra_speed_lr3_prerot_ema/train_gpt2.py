...
EXP_LEARNING_RATE = 0.003
EXP_WARMUP_ITERS = 256
EXP_WARMDOWN_ITERS = 1024
EXP_EMA_DECAY = 0.995
EXP_INTERMEDIATE_VAL_BATCHES = 8
EXP_LOG_EVERY = 128
# --------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    def get_lr(it):
        ...
        if it < EXP_WARMUP_ITERS:
            return EXP_LEARNING_RATE * (it + 1) / EXP_WARMUP_ITERS
        elif it < args.num_iterations - EXP_WARMDOWN_ITERS:
            return EXP_LEARNING_RATE
        else:
            decay_ratio = (args.num_iterations - it) / EXP_WARMDOWN_ITERS
            return EXP_LEARNING_RATE * decay_ratio
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            if last_step:
                ...
                orig_data = [p.data.clone() for p in model_params]
                ...
                # Restore and evaluate raw weights
                for p, orig in zip(model_params, orig_data):
                    p.data.copy_(orig)
                val_loader.reset()
                with torch.no_grad():
                    val_loss_raw = 0.0
                ...
                final_val = min(ema_val_loss, raw_val_loss)
                print0(f"step:{step}/{args.num_iterations} | val loss {final_val:.6f} (ema:{ema_val_loss:.6f} raw:{raw_val_loss:.6f})")
                if master_process and logfile is not None:
                    with open(logfile, "a") as f:
                        f.write("s:%d val:%f\n" % (step, final_val))
            else:
                current_val_steps = EXP_INTERMEDIATE_VAL_BATCHES
                with torch.no_grad():
                    val_loss = 0.0
                    for _ in range(current_val_steps):
                        x_val, y_val = val_loader.next_batch()
        ...
        with torch.no_grad():
            for ema_p, p in zip(ema_params, raw_model.parameters()):
                ema_p.lerp_(p.data, 1.0 - EXP_EMA_DECAY)
        # --------------- TRAINING SECTION END -------------------

        # Diagnostics - only sync and log periodically
        if step % EXP_LOG_EVERY == 0 or step < 10:
            torch.cuda.synchronize()
