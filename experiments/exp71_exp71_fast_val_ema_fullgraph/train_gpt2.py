...
if __name__ == "__main__":
    ...
    for p in raw_model.parameters():
        if p.requires_grad:
            ema_params.append((p, p.data.clone()))
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            if last_step:
                # Full validation with both raw and EMA weights
                with torch.no_grad():
                    val_loss_raw = 0.0
                ...
                # Restore raw weights
                for (p, ema), orig in zip(ema_params, orig_data):
                    p.data.copy_(orig)
                ...
                actual_val_steps = min(FAST_VAL_BATCHES, val_steps)
                with torch.no_grad():
                    val_loss = 0.0
                ...
                print0(f"step:{step}/{args.num_iterations} | val loss {val_loss:.6f} (fast, {actual_val_steps} batches)")
        ...
        with torch.no_grad():
            for p, ema in ema_params:
                ema.lerp_(p.data, 1.0 - EMA_DECAY)
