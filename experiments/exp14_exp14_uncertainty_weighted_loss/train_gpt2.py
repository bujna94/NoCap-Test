...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, return_logits=True):
        ...
        if targets is not None:
            ...
            # Compute per-token uncertainty weights based on prediction entropy
            with torch.no_grad():
                probs = F.softmax(logits, dim=-1)
                # Compute entropy: -sum(p * log(p))
                # High entropy = high uncertainty = should weight more
                entropy = -torch.sum(probs * torch.log(probs + 1e-10), dim=-1)
                # Normalize entropy to [0, 1] range approximately
                # Max entropy for uniform distribution over vocab_size
                max_entropy = math.log(logits.size(-1))
                normalized_entropy = entropy / max_entropy
                # Weight uncertain predictions more (entropy near 1.0 gets weight > 1.0)
                # Weight confident predictions less (entropy near 0.0 gets weight < 1.0)
                # Use exponential scaling: weight = exp(alpha * (normalized_entropy - 0.5))
                # This gives weight ~0.6 for confident (entropy=0), ~1.0 for medium (entropy=0.5), ~1.6 for uncertain (entropy=1.0)
                alpha = 1.0
                uncertainty_weights = torch.exp(alpha * (normalized_entropy - 0.5))
            ...
            weighted_losses = token_losses * uncertainty_weights.view(-1)
            
            # Average the weighted losses
            # Handle ignore_index by masking
            mask = (targets.view(-1) != -1).float()
            loss = (weighted_losses * mask).sum() / (mask.sum() + 1e-10)
        else:
