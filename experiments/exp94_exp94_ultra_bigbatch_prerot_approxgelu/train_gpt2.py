...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_iterations", type=int, default=4590)
    parser.add_argument("--learning_rate", type=float, default=0.0036)
    parser.add_argument("--warmup_iters", type=int, default=200)
    parser.add_argument("--warmdown_iters", type=int, default=800)
    ...
    num_vocab = 50257
    model_config = {
        "d12": GPTConfig(vocab_size=num_vocab, n_layer=12, n_head=12, n_embd=768, max_seq_len=T),
        "d24": GPTConfig(vocab_size=num_vocab, n_layer=24, n_head=16, n_embd=1024, max_seq_len=T),
        "d36": GPTConfig(vocab_size=num_vocab, n_layer=36, n_head=20, n_embd=1280, max_seq_len=T),
        "d48": GPTConfig(vocab_size=num_vocab, n_layer=48, n_head=25, n_embd=1600, max_seq_len=T),
    }[args.model]
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            with torch.no_grad():
                val_loss = 0.0
                current_val_steps = val_steps if last_step else INTERMEDIATE_VAL_BATCHES
            ...
            if use_ema:
                ...
                for p, orig_p in zip(raw_model.parameters(), orig_params):
                    p.data.copy_(orig_p)
                print0(f"step:{step}/{args.num_iterations} | val loss (ema) {ema_val:.6f} | train_time:{training_time_ms/1000:.2f}s")
                val_loss = torch.tensor(ema_val)
            else:
                print0(f"step:{step}/{args.num_iterations} | val loss {val_loss:.6f} | train_time:{training_time_ms/1000:.2f}s")

            if master_process and logfile is not None:
                with open(logfile, "a") as f:
                    f.write("s:%d val:%f\n" % (step, val_loss if isinstance(val_loss, float) else val_loss.item()))
