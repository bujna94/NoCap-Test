...
# -----------------------------------------------------------------------------
# Adaptive gradient clipping helper

class AdaptiveGradientClipper:
    def __init__(self, model, window_size=100, percentile=95.0, clip_multiplier=2.0):
        ...
        for name, param in model.named_parameters():
            if param.requires_grad:
                if 'wte' in name:
                    ...
                    self.param_groups['output'].append(param)
                elif 'attn' in name or 'c_attn' in name or 'rotary' in name:
                    self.param_groups['attention'].append(param)
                elif 'mlp' in name or 'c_fc' in name:
                    self.param_groups['mlp'].append(param)
        
        # History buffers for each group
        self.grad_norm_history = {
            group_name: deque(maxlen=window_size) 
            for group_name in self.param_groups.keys()
        }
        
    def clip_gradients(self):
        """Clip gradients based on historical percentiles per parameter group"""
        for group_name, params in self.param_groups.items():
            ...
            # Store in history
            self.grad_norm_history[group_name].append(total_norm)
            
            # Compute adaptive clipping threshold
            if len(self.grad_norm_history[group_name]) >= 10:  # need some history
                history_array = np.array(list(self.grad_norm_history[group_name]))
                threshold = np.percentile(history_array, self.percentile) * self.clip_multiplier
                
                # Clip if needed
                if total_norm > threshold and threshold > 0:
...
if __name__ == "__main__":
    ...
    grad_clipper = AdaptiveGradientClipper(raw_model, window_size=100, percentile=95.0, clip_multiplier=2.0)
