...
if __name__ == "__main__":
    ...
    # EMA setup: single shadow copy, decay=0.998
    EMA_DECAY = 0.998
    model_param_list = list(raw_model.parameters())
    ema_params = [p.data.clone() for p in model_param_list]
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            # EMA evaluation ONLY on the last step to minimize time overhead
            if last_step and step > 0:
                # Zero-copy swap: exchange .data pointers
                for i, p in enumerate(model_param_list):
                    tmp = p.data
                    p.data = ema_params[i]
                    ema_params[i] = tmp
                ...
                final_val_loss = min(raw_val_loss, ema_val_loss)
                print0(f"step:{step}/{args.num_iterations} | val loss {final_val_loss:.6f} | raw {raw_val_loss:.6f} | ema {ema_val_loss:.6f}")
            else:
                final_val_loss = raw_val_loss
                print0(f"step:{step}/{args.num_iterations} | val loss {final_val_loss:.6f}")
        ...
        # Update EMA - minimal overhead (~1ms per step)
        with torch.no_grad():
            for i, p in enumerate(model_param_list):
