...
if __name__ == "__main__":
    ...
    total_micro_tokens = args.batch_size * (args.grad_accumulation_steps // ddp_world_size)
    grad_accumulation_steps = max(1, total_micro_tokens // MICRO_BATCH)
    if total_micro_tokens % MICRO_BATCH != 0:
        grad_accumulation_steps = (total_micro_tokens + MICRO_BATCH - 1) // MICRO_BATCH

    device = f"cuda:{ddp_local_rank}"
    torch.cuda.set_device(device)
    master_process = ddp_rank == 0
    print0(f"Original B={args.batch_size}, grad_accum={args.grad_accumulation_steps}, world={ddp_world_size}")
    print0(f"New B={MICRO_BATCH}, grad_accum={grad_accumulation_steps}")

    tokens_per_iter = MICRO_BATCH * T * ddp_world_size * grad_accumulation_steps
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            if last_step:
                ...
                use_batches = INTERMEDIATE_VAL_BATCHES
                with torch.no_grad():
                    val_loss = 0.0
        ...
        if step % LOG_EVERY == 0 or step < 2:
            ...
            print0(
                f"step:{step}/{args.num_iterations} | loss {loss.item() * grad_accumulation_steps:.6f} | train_time:{approx_training_time_ms/1000:.2f}s | step_avg:{approx_training_time_ms/(step+1):.2f}ms"
            )
            if master_process and logfile is not None:
                with open(logfile, "a") as f:
                    f.write("s:%d trn:%f\n" % (step, loss.item() * grad_accumulation_steps))
