...
if __name__ == "__main__":
    ...
    WARMUP_STEPS = 256
    WARMDOWN_STEPS = 1024
    CYCLIC_AMPLITUDE = 0.10
    CYCLIC_PERIOD = 128
    EMA_DECAY_CONSTANT = 0.995
    ...
    def get_lr(it):
        ...
        if it < WARMUP_STEPS:
            ...
            cycle_pos = (it - WARMUP_STEPS) % CYCLIC_PERIOD
            half_period = CYCLIC_PERIOD / 2.0
            if cycle_pos < half_period:
                frac = cycle_pos / half_period
                scale = (1.0 - CYCLIC_AMPLITUDE) + 2.0 * CYCLIC_AMPLITUDE * frac
            else:
                frac = (cycle_pos - half_period) / half_period
                scale = (1.0 + CYCLIC_AMPLITUDE) - 2.0 * CYCLIC_AMPLITUDE * frac
    ...
    warmdown_start = args.num_iterations - WARMDOWN_STEPS

    for step in range(args.num_iterations + 1):
        last_step = step == args.num_iterations

        # Only validate at step 0 and final step for maximum speed
        if last_step or step == 0:
            ...
            if last_step:
                # Full validation with raw model
                with torch.no_grad():
                    val_loss_raw = 0.0
                ...
                # Full validation with EMA model
                orig_params = []
                for ema_p, p in zip(ema_params, raw_model.parameters()):
        ...
        if step % LOG_EVERY == 0 or step < 2:
            ...
            print0(f"step:{step}/{args.num_iterations} | loss {lossf:.6f} | lr {lr:.6f} | train_time:{approx_training_time_ms/1000:.2f}s | step_avg:{approx_training_time_ms/(step+1):.2f}ms")
            if master_process and logfile is not None:
                with open(logfile, "a") as f:
                    f.write("s:%d trn:%f\n" % (step, lossf))
