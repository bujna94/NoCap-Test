...
if __name__ == "__main__":
    ...
    # ---- EMA setup: pre-build flat parameter lists for speed ----
    EMA_DECAY = 0.998
    param_list = list(raw_model.parameters())
    ema_list = [p.data.clone() for p in param_list]
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            if last_step:
                # Final step: evaluate both raw and EMA, report the better one
                # First evaluate raw weights
                with torch.no_grad():
                    val_loss_raw = 0.0
                ...
                if ema_val < raw_val:
                    val_loss_report = ema_val
                    best_label = "ema"
                else:
                    val_loss_report = raw_val
                    best_label = "raw"
                print0(f"step:{step}/{args.num_iterations} | val loss {val_loss_report:.6f} | best={best_label} (raw={raw_val:.6f} ema={ema_val:.6f})")
            else:
                # Non-final steps: evaluate raw weights only (zero overhead)
                with torch.no_grad():
                    val_loss = 0.0
                    for _ in range(val_steps):
                        x_val, y_val = val_loader.next_batch()
        ...
        # Update EMA - use pre-built lists for minimal overhead
        with torch.no_grad():
            for i in range(len(param_list)):
                ema_list[i].lerp_(param_list[i].data, 1.0 - EMA_DECAY)
