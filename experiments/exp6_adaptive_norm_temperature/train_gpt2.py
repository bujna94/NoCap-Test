...
class AdaptiveRMSNorm(nn.Module):
    """RMSNorm with adaptive temperature based on gradient statistics"""
    def __init__(self, dim, eps=1e-6):
        super().__init__()
        self.eps = eps
        self.dim = dim
        self.weight = nn.Parameter(torch.ones(dim))
        # Temperature parameter that modulates normalization strength
        self.register_buffer('temperature', torch.ones(1))
        # Running statistics for gradient CV
        self.register_buffer('grad_mean_ema', torch.zeros(1))
        self.register_buffer('grad_std_ema', torch.ones(1))
        self.ema_decay = 0.95
        
    def forward(self, x0):
        x = x0.float()
        # Apply temperature-modulated normalization
        norm_factor = torch.rsqrt(x.pow(2).mean(-1, keepdim=True) + self.eps)
        # Temperature > 1 means stronger normalization, < 1 means weaker
        x = x * (norm_factor ** self.temperature) * self.weight
        return x.type_as(x0)
    
    def update_temperature(self, grad_stats):
        """Update temperature based on gradient coefficient of variation"""
        if grad_stats is not None and not torch.isnan(grad_stats).any():
            # Update EMA of gradient statistics
            self.grad_mean_ema = self.ema_decay * self.grad_mean_ema + (1 - self.ema_decay) * grad_stats
            # Temperature is inversely proportional to gradient stability
            # High CV (noisy) -> higher temperature (stronger norm)
            # Low CV (stable) -> lower temperature (weaker norm)
            # Clamp temperature to reasonable range [0.8, 1.2]
            cv = torch.abs(self.grad_std_ema / (self.grad_mean_ema + 1e-8))
            self.temperature = torch.clamp(1.0 + 0.1 * (cv - 1.0), 0.8, 1.2)
...
class Block(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.norm1 = AdaptiveRMSNorm(config.n_embd)
        self.norm2 = AdaptiveRMSNorm(config.n_embd)
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.final_norm = AdaptiveRMSNorm(config.n_embd)
    ...
    def update_norm_temperatures(self):
        """Update adaptive normalization temperatures based on gradient statistics"""
        for block in self.transformer.h:
            # Compute gradient statistics for each normalization layer
            if block.norm1.grad_mean_ema.device == torch.device('cpu'):
                continue  # Skip if not on GPU yet
            
            # Get gradient stats from attention path
            attn_params = list(block.attn.parameters())
            if len(attn_params) > 0 and attn_params[0].grad is not None:
                grads = torch.cat([p.grad.flatten() for p in attn_params if p.grad is not None])
                if len(grads) > 0:
                    grad_std = grads.std()
                    grad_mean = grads.abs().mean()
                    cv = grad_std / (grad_mean + 1e-8)
                    block.norm1.update_temperature(cv)
            
            # Get gradient stats from MLP path
            mlp_params = list(block.mlp.parameters())
            if len(mlp_params) > 0 and mlp_params[0].grad is not None:
                grads = torch.cat([p.grad.flatten() for p in mlp_params if p.grad is not None])
                if len(grads) > 0:
                    grad_std = grads.std()
                    grad_mean = grads.abs().mean()
                    cv = grad_std / (grad_mean + 1e-8)
                    block.norm2.update_temperature(cv)
...
if __name__ == "__main__":
    ...
    for step in range(args.num_iterations + 1):
        ...
        if step % 10 == 0:
            raw_model.update_norm_temperatures()
