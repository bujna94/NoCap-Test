...
class EMAWeights:
    ...
    def __init__(self, model, decay=0.995):
        self.decay = decay
        # Store EMA weights as plain tensors (not parameters) on the same device
        self.shadow = {}
    ...
    def apply_shadow(self, model):
        """Replace model params with EMA params, return backup of original params."""
        backup = {}
        for name, param in model.named_parameters():
...
if __name__ == "__main__":
    ...
    # Initialize EMA after model is on GPU but using the raw (unwrapped) model's parameters
    # We start EMA updates after warmup to avoid averaging over random early weights
    ema_decay = 0.995
    ema_start_step = 200  # start EMA after initial training stabilizes
    ema_shadow = None  # will be initialized lazily
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            # If EMA is available, swap in EMA weights for evaluation
            use_ema = ema_shadow is not None
            backup = None
            if use_ema:
                # Swap EMA weights into the raw model for evaluation
                backup = {}
            ...
            print0(f"step:{step}/{args.num_iterations} | val loss {val_loss:.6f} | ema:{use_ema}")
            if master_process:
                if args.log_wandb:
        ...
        if step >= ema_start_step:
            if ema_shadow is None:
                # Initialize EMA shadow with current weights
                ema_shadow = {}
                for name, param in raw_model.named_parameters():
                    ema_shadow[name] = param.data.clone()
                print0(f"EMA initialized at step {step}")
            else:
                # Update EMA: shadow = decay * shadow + (1-decay) * param
                for name, param in raw_model.named_parameters():
                    ema_shadow[name].mul_(ema_decay).add_(param.data, alpha=1.0 - ema_decay)
