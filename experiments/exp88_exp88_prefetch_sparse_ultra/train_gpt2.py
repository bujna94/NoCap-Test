...
class PrefetchLoader:
    """Wraps a DistributedDataLoader to prefetch the next batch asynchronously."""
    ...
    def prefetch(self):
        with torch.cuda.stream(self.stream):
            self.next_x, self.next_y = self.loader.next_batch()
...
if __name__ == "__main__":
    ...
    print0(f"micro batch size: {B}, grad accum steps (per GPU): {grad_accumulation_steps}")

    ctx = torch.amp.autocast(device_type="cuda", dtype=torch.bfloat16)

    raw_train_loader = DistributedDataLoader(args.input_bin, B, T, ddp_rank, ddp_world_size)
    val_loader = None
    ...
    # Set up prefetch loader
    train_loader = PrefetchLoader(raw_train_loader)
    # Get the first batch and start prefetching
    x, y = raw_train_loader.next_batch()
