...
if __name__ == "__main__":
    ...
    # Dynamic parameter freezing tracking
    param_update_history = {}  # maps parameter name to list of recent update norms
    frozen_params = set()  # set of frozen parameter names
    window_size = 50  # track last 50 updates
    freeze_threshold = 1e-5  # freeze if average update norm is below this
    start_freezing_step = 1000  # don't freeze anything before this step
    check_every = 100  # check for freezing every N steps
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            if master_process:
                if args.log_wandb:
                    ...
                    wandb.log({"frozen_params_count": len(frozen_params)}, step=step * tokens_per_iter)
        ...
        # Store parameter values before update to compute update norms
        if step >= start_freezing_step and step % check_every == 0:
            param_snapshot = {}
            for name, param in raw_model.named_parameters():
                if name not in frozen_params and param.requires_grad:
        ...
        # Compute parameter update norms and potentially freeze parameters
        if step >= start_freezing_step and step % check_every == 0:
            for name, param in raw_model.named_parameters():
                if name not in frozen_params and param.requires_grad and name in param_snapshot:
                    # Compute L2 norm of the parameter update
                    update_norm = (param.data - param_snapshot[name]).norm().item()
                    
                    # Initialize or update history
                    if name not in param_update_history:
                        param_update_history[name] = []
                    param_update_history[name].append(update_norm)
                    
                    # Keep only recent history
                    if len(param_update_history[name]) > window_size:
                        param_update_history[name].pop(0)
                    
                    # Check if we should freeze this parameter
                    if len(param_update_history[name]) >= window_size:
                        avg_update_norm = sum(param_update_history[name]) / len(param_update_history[name])
                        if avg_update_norm < freeze_threshold:
                            frozen_params.add(name)
                            param.requires_grad = False
                            if master_process and step % 500 == 0:
                                print0(f"Freezing parameter: {name} (avg update norm: {avg_update_norm:.2e})")
        ...
        print0(
            f"step:{step}/{args.num_iterations} | loss {lossf:.6f} | train_time:{approx_training_time_ms/1000:.2f}s | step_avg:{approx_training_time_ms/(step+1):.2f}ms | frozen:{len(frozen_params)}"
        )
        # log to logile
