...
def slerp_flat(t_val, v0, v1):
    ...
    interp = (torch.sin((1.0 - t_val) * omega) / sin_omega) * v0 + (torch.sin(t_val * omega) / sin_omega) * v1
    return interp


def slerp_state_dicts(sd0, sd1, t_val):
    """Slerp between two state dicts, parameter by parameter."""
    result = {}
    for key in sd0:
        p0 = sd0[key]
        p1 = sd1[key]
        p0_flat = p0.reshape(-1).float()
        p1_flat = p1.reshape(-1).float()
        interp = slerp_flat(t_val, p0_flat, p1_flat)
        result[key] = interp.reshape(p0.shape).to(p0.dtype)
    return result


def lerp_state_dicts(sd0, sd1, t_val):
    """Linear interpolation between two state dicts."""
    result = {}
    for key in sd0:
        result[key] = ((1.0 - t_val) * sd0[key].float() + t_val * sd1[key].float()).to(sd0[key].dtype)
    return result


def avg_state_dicts(sd_list, weights=None):
    """Compute weighted average of a list of state dicts."""
    if weights is None:
        weights = [1.0 / len(sd_list)] * len(sd_list)
    else:
        total = sum(weights)
        weights = [w / total for w in weights]
    result = {}
    for key in sd_list[0]:
        acc = torch.zeros_like(sd_list[0][key], dtype=torch.float32)
        for sd, w in zip(sd_list, weights):
            acc += w * sd[key].float()
        result[key] = acc.to(sd_list[0][key].dtype)
    return result
...
if __name__ == "__main__":
    ...
    # SWA: collect snapshots every 8 steps over the final 1024 steps
    SWA_START_STEP = max(0, args.num_iterations - 1024)
    SWA_INTERVAL = max(1, 8)
    swa_snapshots = []  # list of state_dict copies (on CPU to save GPU memory)

    # EMA: two shadows with different decays
    EMA_DECAYS = [0.998, 0.999]
    ema_shadows = []
    for _ in EMA_DECAYS:
        shadow = {}
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            # Determine which weight sets to evaluate
            candidates = {}  # name -> state_dict

            # Always evaluate current weights
            candidates['current'] = {name: param.data.clone() for name, param in raw_model.named_parameters()}

            # Add EMA candidates
            for i, decay in enumerate(EMA_DECAYS):
                candidates[f'ema_{decay}'] = {k: v.clone() for k, v in ema_shadows[i].items()}

            # Add SWA candidates if we have snapshots
            if len(swa_snapshots) >= 4:
                # Uniform SWA over all snapshots
                candidates['swa_uniform'] = avg_state_dicts(swa_snapshots)

                # Uniform SWA over last half of snapshots
                half = len(swa_snapshots) // 2
                if half >= 2:
                    candidates['swa_last_half'] = avg_state_dicts(swa_snapshots[half:])

                # Linearly-weighted SWA (more weight on later snapshots)
                n_snap = len(swa_snapshots)
                lin_weights = [float(i + 1) for i in range(n_snap)]
                candidates['swa_linear'] = avg_state_dicts(swa_snapshots, weights=lin_weights)

                # Quadratically-weighted SWA
                quad_weights = [float((i + 1) ** 2) for i in range(n_snap)]
                candidates['swa_quadratic'] = avg_state_dicts(swa_snapshots, weights=quad_weights)

            # Generate slerp interpolations between select pairs
            # Only do this if we have enough base candidates and it's the last evaluation
            base_candidate_names = [k for k in candidates.keys() if k != 'current']
            if last_step and len(base_candidate_names) >= 2:
                # Slerp between EMA and SWA candidates
                slerp_pairs = []
                ema_names = [k for k in base_candidate_names if k.startswith('ema_')]
                swa_names = [k for k in base_candidate_names if k.startswith('swa_')]
                for en in ema_names:
                    for sn in swa_names[:2]:  # limit pairs to avoid too many candidates
                        slerp_pairs.append((en, sn))
                # Also slerp between current and best-looking averages
                for sn in swa_names[:2]:
                    slerp_pairs.append(('current', sn))

                for (n1, n2) in slerp_pairs:
                    for alpha in [0.3, 0.5, 0.7]:
                        try:
                            interp_sd = slerp_state_dicts(candidates[n1], candidates[n2], alpha)
                            candidates[f'slerp_{n1}_{n2}_{alpha}'] = interp_sd
                        except Exception:
                            pass  # skip if slerp fails for any reason

            # Evaluate all candidates, find best
            best_val_loss = float('inf')
            best_name = 'current'

            # Save current weights to restore after evaluation
            original_sd = {name: param.data.clone() for name, param in raw_model.named_parameters()}

            for cand_name, cand_sd in candidates.items():
                # Load candidate weights
                with torch.no_grad():
                    for name, param in raw_model.named_parameters():
                        param.data.copy_(cand_sd[name])
                ...
                if master_process:
                    print0(f"  candidate '{cand_name}': val_loss = {cand_loss_val:.6f}")
            ...
            with torch.no_grad():
                for name, param in raw_model.named_parameters():
                    param.data.copy_(original_sd[name])

            val_loss = best_val_loss

            # log to console and to file
            print0(f"step:{step}/{args.num_iterations} | val loss {val_loss:.6f} | best candidate: {best_name}")
            if master_process:
                if args.log_wandb:
            ...
            # Clean up candidate dicts to free memory
            del candidates
            del original_sd
        ...
        with torch.no_grad():
            for i, decay in enumerate(EMA_DECAYS):
                for name, param in raw_model.named_parameters():
                    ema_shadows[i][name].mul_(decay).add_(param.data, alpha=1.0 - decay)

        # Collect SWA snapshot if in the window
        if step >= SWA_START_STEP and (step - SWA_START_STEP) % SWA_INTERVAL == 0:
            snapshot = {name: param.data.clone() for name, param in raw_model.named_parameters()}
