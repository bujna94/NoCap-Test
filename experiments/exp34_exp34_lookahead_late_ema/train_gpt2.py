...
# Lookahead hyperparameters (hardcoded)
LOOKAHEAD_K = 5  # sync every k steps
LOOKAHEAD_ALPHA = 0.5  # interpolation factor toward fast weights

# Late EMA hyperparameters
EMA_DECAY = 0.9995
EMA_START_STEP = 4256  # start EMA in the final 512 steps (4768 - 512)
...
if __name__ == "__main__":
    ...
    # Initialize Lookahead slow weights as a copy of current model parameters
    slow_weights = {}
    for name, param in raw_model.named_parameters():
        slow_weights[name] = param.data.clone()

    # Initialize EMA weights (will be populated when EMA starts)
    ema_weights = None
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            # If EMA is initialized, swap in EMA weights for validation
            use_ema = ema_initialized and ema_weights is not None
            if use_ema:
                # Save current (fast) weights and swap in EMA weights
                saved_weights = {}
                for name, param in raw_model.named_parameters():
            ...
            # Restore fast weights after validation
            if use_ema:
                for name, param in raw_model.named_parameters():
        ...
        # --- Lookahead: every K steps, sync slow weights toward fast weights ---
        if (step + 1) % LOOKAHEAD_K == 0:
            with torch.no_grad():
                for name, param in raw_model.named_parameters():
                    if name in slow_weights:
                        # slow = slow + alpha * (fast - slow)
                        slow_weights[name].add_(
                            LOOKAHEAD_ALPHA * (param.data - slow_weights[name])
                        )
                        # Set fast weights back to slow weights
                        param.data.copy_(slow_weights[name])

        # --- Late EMA: update EMA weights in final phase ---
        if step >= EMA_START_STEP:
            with torch.no_grad():
                if not ema_initialized:
                    ...
                    for name, param in raw_model.named_parameters():
                        if name in ema_weights:
                            ema_weights[name].mul_(EMA_DECAY).add_(
                                (1.0 - EMA_DECAY) * param.data
                            )
