...
HARDCODED_LR = 0.004
HARDCODED_WARMUP = 128
HARDCODED_WARMDOWN = 512
EMA_DECAY = 0.997
MICRO_BATCH = 96
VOCAB_SIZE = 50304
...
if __name__ == "__main__":
    ...
    effective_grad_accum = total_tokens_per_step // (B * T)
    if effective_grad_accum < 1:
        effective_grad_accum = 1
    assert effective_grad_accum % ddp_world_size == 0
    grad_accum_per_gpu = effective_grad_accum // ddp_world_size

    tokens_per_iter = B * T * ddp_world_size * grad_accum_per_gpu
    print0(f"tokens per iteration: {tokens_per_iter:,} (B={B}, grad_accum_per_gpu={grad_accum_per_gpu})")
    ...
    def get_lr(it):
        ...
        if it < HARDCODED_WARMUP:
            return HARDCODED_LR * (it + 1) / HARDCODED_WARMUP
        elif it < args.num_iterations - HARDCODED_WARMDOWN:
            return HARDCODED_LR
        else:
            decay_ratio = (args.num_iterations - it) / HARDCODED_WARMDOWN
            return HARDCODED_LR * decay_ratio
    ...
    for step in range(args.num_iterations + 1):
        ...
        # Only validate at step 0 and final step - no intermediate validation
        do_val = (step == 0 or last_step) and args.val_loss_every > 0
        ...
        if do_val:
            ...
            if last_step:
                # Evaluate both raw and EMA, pick best
                with torch.no_grad():
                    val_loss_raw = 0.0
                ...
                print0(f"step:{step}/{args.num_iterations} | val loss raw:{val_loss_raw:.6f} ema:{val_loss_ema:.6f} best:{val_loss:.6f}")
            else:
                with torch.no_grad():
                    val_loss = 0.0
                    for _ in range(val_steps):
