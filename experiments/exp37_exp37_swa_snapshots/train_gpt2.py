...
# SWA configuration - hardcoded
SWA_START_BEFORE_END = 512  # start collecting snapshots this many steps before end
SWA_SNAPSHOT_EVERY = 64     # collect a snapshot every N steps
...
if __name__ == "__main__":
    ...
    # ---- SWA: Initialize snapshot storage ----
    # We'll store weight snapshots on CPU to avoid GPU memory issues
    ...
    swa_avg_state = None  # the averaged state dict
    print0(f"SWA: will collect snapshots every {SWA_SNAPSHOT_EVERY} steps starting at step {swa_start_step}")
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            # Decide whether to use SWA weights for evaluation
            use_swa = last_step and swa_avg_state is not None
            original_state = None
            ...
            if use_swa:
                ...
                for name, param in raw_model.named_parameters():
                    if name in swa_avg_state:
                        param.data.copy_(swa_avg_state[name].to(param.device))
                print0(f"SWA: Using averaged weights from {len(swa_snapshots)} snapshots for final evaluation")
            ...
            # If we used SWA and the result is good, also check without SWA
            if use_swa:
                swa_val_loss = val_loss.item()
                # Restore original weights and evaluate without SWA too
                for name, param in raw_model.named_parameters():
                ...
                with torch.no_grad():
                    ...
                    for _ in range(val_steps):
                        x_val, y_val = val_loader.next_batch()
                        _, loss = model(x_val, y_val, return_logits=False)
                        val_loss_no_swa += loss
                    dist.all_reduce(val_loss_no_swa, op=dist.ReduceOp.AVG)
                    val_loss_no_swa /= val_steps
                
                no_swa_val = val_loss_no_swa.item()
                print0(f"SWA val loss: {swa_val_loss:.6f} | No-SWA val loss: {no_swa_val:.6f}")
                
                # Use the better one
                if swa_val_loss < no_swa_val:
                    val_loss = torch.tensor(swa_val_loss)
                    print0(f"SWA is better, using SWA loss")
                else:
                    val_loss = val_loss_no_swa
                    print0(f"No-SWA is better, using regular loss")
        ...
        if step >= swa_start_step and (step - swa_start_step) % SWA_SNAPSHOT_EVERY == 0:
            # Save a snapshot of current weights to CPU
            snapshot = {}
            for name, param in raw_model.named_parameters():
            ...
            if swa_avg_state is None:
                swa_avg_state = {name: t.clone() for name, t in snapshot.items()}
            else:
                n = len(swa_snapshots)
                for name in swa_avg_state:
                    # Running average: avg = avg * (n-1)/n + new/n
                    swa_avg_state[name].mul_((n - 1.0) / n).add_(snapshot[name], alpha=1.0 / n)
