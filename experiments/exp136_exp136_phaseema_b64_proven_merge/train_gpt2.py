...
# --- Hardcoded speed/quality parameters ---
PEAK_LR = 0.003
EMA_DECAY_NORMAL = 0.998
EMA_DECAY_WARMDOWN = 0.9995
MICRO_BATCH = 64  # larger micro-batch for speed
INTERMEDIATE_VAL_BATCHES = 4  # reduced from 64
...
if __name__ == "__main__":
    ...
    orig_tokens_per_step = args.batch_size * args.grad_accumulation_steps * T
    new_grad_accum = max(1, orig_tokens_per_step // (B * T))
    # Round up to be divisible by world size
    if new_grad_accum % ddp_world_size != 0:
        new_grad_accum = ((new_grad_accum // ddp_world_size) + 1) * ddp_world_size
    ...
    print0(f"Micro-batch: {B}, grad_accum_per_gpu: {grad_accum_per_gpu}, total tokens/step: {B * T * ddp_world_size * grad_accum_per_gpu}")
    ...
    for step in range(num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            if last_step:
                ...
                print0(f"step:{step}/{num_iterations} | val loss EMA {val_loss:.6f}")
                ...
                actual_val_steps = min(INTERMEDIATE_VAL_BATCHES, val_steps)
                with torch.no_grad():
                    val_loss = 0.0
                    for _ in range(actual_val_steps):
                        x_val, y_val = val_loader.next_batch()
                        _, loss = model(x_val, y_val, return_logits=False)
                        val_loss += loss
                    dist.all_reduce(val_loss, op=dist.ReduceOp.AVG)
                    val_loss /= actual_val_steps
