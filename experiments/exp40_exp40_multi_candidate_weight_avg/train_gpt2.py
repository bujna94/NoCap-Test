...
if __name__ == "__main__":
    ...
    # =========================================================================
    # SWA: collect snapshots every 4 steps over the final 1024 steps
    SWA_START_STEP = args.num_iterations - 1024  # step 3744
    SWA_EVERY = 4  # snapshot every 4 steps = 256 snapshots over 1024 steps
    
    # EMA: maintain 3 EMA shadows with different decay rates
    EMA_DECAYS = [0.998, 0.999, 0.9995]
    
    # Initialize EMA shadows as flat lists of tensors (completely separate from model)
    ema_shadows = []
    for decay in EMA_DECAYS:
        shadow = [p.data.clone() for p in raw_model.parameters()]
        ema_shadows.append(shadow)
    
    # SWA accumulator: running sum of parameters
    swa_sum = None
    swa_count = 0
    
    # Weighted SWA accumulator: weight increases linearly with snapshot index
    wswa_sum = None
    wswa_weight_total = 0.0
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            original_params = [p.data.clone() for p in raw_model.parameters()]
            
            # Try each EMA shadow
            for ema_idx, (decay, shadow) in enumerate(zip(EMA_DECAYS, ema_shadows)):
                # Load EMA weights into model
                for p, s in zip(raw_model.parameters(), shadow):
                    p.data.copy_(s)
                ...
                ema_loss = val_loss_ema.item()
                if ema_loss < best_val_loss:
                    best_val_loss = ema_loss
                    best_source = f"ema_{decay}"
            
            # Try SWA if we have snapshots
            if swa_count > 0:
                for p, s in zip(raw_model.parameters(), swa_sum):
                    p.data.copy_(s / swa_count)
                ...
                swa_loss = val_loss_swa.item()
                if swa_loss < best_val_loss:
                    best_val_loss = swa_loss
                    best_source = "swa"
            
            # Try weighted SWA if we have snapshots
            if wswa_weight_total > 0:
                for p, s in zip(raw_model.parameters(), wswa_sum):
                    p.data.copy_(s / wswa_weight_total)
                ...
                with torch.no_grad():
                    ...
                    for _ in range(val_steps):
                        ...
                        _, loss = model(x_val, y_val, return_logits=False)
                        val_loss_wswa += loss
                    dist.all_reduce(val_loss_wswa, op=dist.ReduceOp.AVG)
                    val_loss_wswa /= val_steps
                
                wswa_loss = val_loss_wswa.item()
                if wswa_loss < best_val_loss:
                    best_val_loss = wswa_loss
                    best_source = "wswa"
            ...
            print0(f"step:{step}/{args.num_iterations} | val loss {val_loss:.6f} | source:{best_source}")
            if master_process:
                if args.log_wandb:
        ...
        # Update EMA shadows (after optimizer step)
        with torch.no_grad():
            for ema_idx, decay in enumerate(EMA_DECAYS):
                shadow = ema_shadows[ema_idx]
                for i, p in enumerate(raw_model.parameters()):
                    shadow[i].mul_(decay).add_(p.data, alpha=1.0 - decay)
        
        # Collect SWA snapshots
        if step >= SWA_START_STEP and (step - SWA_START_STEP) % SWA_EVERY == 0:
            with torch.no_grad():
                if swa_sum is None:
                    swa_sum = [p.data.clone() for p in raw_model.parameters()]
                    wswa_sum = [p.data.clone() for p in raw_model.parameters()]  # weight=1 for first
                    swa_count = 1
                    wswa_weight_total = 1.0
                else:
                    swa_count += 1
                    weight = float(swa_count)  # linearly increasing weight
                    for i, p in enumerate(raw_model.parameters()):
                        swa_sum[i].add_(p.data)
                        wswa_sum[i].add_(p.data * weight)
                    wswa_weight_total += weight
