...
class ProgressiveDataLoader:
    """Data loader that supports variable sequence lengths."""
    def __init__(self, filename_pattern, B, T_max, process_rank, num_processes):
        self.process_rank = process_rank
        self.num_processes = num_processes
        self.B = B
        self.T_max = T_max
        self.current_T = T_max  # will be set by external controller
        ...
        # load and validate all data shards
        ntok_total = np.int64(0)
        for fname in self.files:
            shard_ntok = _peek_data_shard(fname)
            assert shard_ntok >= num_processes * B * T_max + 1
            ntok_total += shard_ntok
        self.ntok_total = ntok_total
        print0(
            f"DataLoader: total number of tokens: {ntok_total:,} across {len(self.files)} files"
    ...
    def reset(self):
        self.current_shard = 0
        self.current_position = self.process_rank * self.B * self.T_max
        self.tokens = _load_data_shard(self.files[self.current_shard])
    ...
    def set_sequence_length(self, T):
        """Update the sequence length for next batch."""
        self.current_T = T
    ...
    def next_batch(self):
        ...
        x = (buf[:-1]).view(B, T)
        y = (buf[1:]).view(B, T)
        # advance by T_max to keep consistent positioning
        self.current_position += B * self.T_max * self.num_processes
        if self.current_position + (B * self.T_max * self.num_processes + 1) > len(self.tokens):
            self.advance()
        return x.cuda(), y.cuda()
...
if __name__ == "__main__":
    ...
    T_min = 32
    T_max = T
    progressive_steps = 2000  # ramp up sequence length over first 2000 steps
    
    def get_sequence_length(step):
        if step >= progressive_steps:
            return T_max
        # Linear ramp from T_min to T_max
        progress = step / progressive_steps
        current_T = int(T_min + (T_max - T_min) * progress)
        # Round to nearest power of 2 for efficiency
        current_T = 2 ** round(math.log2(current_T))
        return min(max(current_T, T_min), T_max)

    # load tokens with progressive loader
    train_loader = ProgressiveDataLoader(args.input_bin, B, T_max, ddp_rank, ddp_world_size)
    val_loader = None
    ...
    for step in range(args.num_iterations + 1):
        ...
        # Get current sequence length for this step
        current_T = get_sequence_length(step)
        train_loader.set_sequence_length(current_T)
        
        # Adjust tokens per iter based on current sequence length
        current_tokens_per_iter = B * current_T * ddp_world_size * args.grad_accumulation_steps
        
        if step % 100 == 0:
            print0(f"step {step}: using sequence length T={current_T}")
        ...
        for micro_step in range(args.grad_accumulation_steps):
            ...
            # Get batch with current sequence length
