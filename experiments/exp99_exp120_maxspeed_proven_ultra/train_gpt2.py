...
if __name__ == "__main__":
    ...
    VAL_EVERY_OVERRIDE = 5000  # Only validate at start and end

    ctx = torch.amp.autocast(device_type="cuda", dtype=torch.bfloat16)

    train_loader = DistributedDataLoader(args.input_bin, B, T, ddp_rank, ddp_world_size)
    val_loader = DistributedDataLoader(args.input_val_bin, B, T, ddp_rank, ddp_world_size)
    tokens_per_iter_val = B * T * ddp_world_size
    val_steps = VAL_TOKENS // tokens_per_iter_val

    # Pre-fetch first training batch
    ...
    # Initialize EMA shadows
    ema_params = [p.data.clone() for p in raw_model.parameters()]

    # Create secondary CUDA stream for data prefetch
    ...
    for step in range(args.num_iterations + 1):
        ...
        # Validation only at start and final step
        if args.val_loss_every > 0 and (step % VAL_EVERY_OVERRIDE == 0 or last_step):
            torch.cuda.synchronize()
        ...
        for micro_step in range(args.grad_accumulation_steps):
            is_last = (micro_step == args.grad_accumulation_steps - 1)
            model.require_backward_grad_sync = is_last
            with ctx:
                _, loss = model(x, y, return_logits=False)
                loss = loss / args.grad_accumulation_steps
            # Start prefetching next batch on secondary stream during backward
            loss.backward()
            # Prefetch next batch
        ...
        # Update EMA (skip during warmup for speed)
        if step >= WARMUP // 2:
            with torch.no_grad():
