...
if __name__ == "__main__":
    ...
    # ---- SWA/EMA configuration (hardcoded) ----
    # Collect snapshots every 8 steps over the final 1024 steps
    SWA_START_STEP = max(args.num_iterations - 1024, 0)  # handle small num_iterations
    SWA_EVERY = 8  # collect every 8 steps
    EMA_DECAY = 0.998
    
    # Storage for SWA snapshots (stored on CPU to save GPU memory)
    swa_snapshots = []  # list of state_dict copies on CPU
    
    # EMA shadow weights on CPU
    ema_state = None
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            # Evaluate with current (unaveraged) weights first
            with torch.no_grad():
            ...
            # Only try weight averaging candidates if we have enough snapshots
            if len(swa_snapshots) >= 4:
                ...
                # Candidate 1: Uniform average of ALL snapshots
                avg_state = {}
                ...
                candidate_states.append(avg_state)
                candidate_labels.append(f"uniform_all_{n_snaps}")
                
                # Candidate 2: Uniform average of last half of snapshots
                half = n_snaps // 2
                if half >= 4:
                    avg_state2 = {}
                    for name in swa_snapshots[0]:
                        avg_state2[name] = torch.zeros_like(swa_snapshots[0][name], dtype=torch.float32)
                        for snap in swa_snapshots[half:]:
                            avg_state2[name] += snap[name].float()
                        avg_state2[name] /= (n_snaps - half)
                    candidate_states.append(avg_state2)
                    candidate_labels.append(f"uniform_last_{n_snaps - half}")
                
                # Candidate 3: Uniform average of last quarter
                quarter = 3 * n_snaps // 4
                if (n_snaps - quarter) >= 4:
                    avg_state3 = {}
                    for name in swa_snapshots[0]:
                        avg_state3[name] = torch.zeros_like(swa_snapshots[0][name], dtype=torch.float32)
                        for snap in swa_snapshots[quarter:]:
                            avg_state3[name] += snap[name].float()
                        avg_state3[name] /= (n_snaps - quarter)
                    candidate_states.append(avg_state3)
                    candidate_labels.append(f"uniform_last_{n_snaps - quarter}")
                
                # Candidate 4: Linearly weighted (more weight to later snapshots)
                lin_state = {}
                weights_lin = torch.arange(1, n_snaps + 1, dtype=torch.float32)
                weights_lin /= weights_lin.sum()
                for name in swa_snapshots[0]:
                    lin_state[name] = torch.zeros_like(swa_snapshots[0][name], dtype=torch.float32)
                    for i, snap in enumerate(swa_snapshots):
                        lin_state[name] += weights_lin[i].item() * snap[name].float()
                candidate_states.append(lin_state)
                candidate_labels.append("linear_weighted")
                
                # Candidate 5: Quadratically weighted (even more weight to later)
                quad_state = {}
                weights_quad = torch.arange(1, n_snaps + 1, dtype=torch.float32) ** 2
                weights_quad /= weights_quad.sum()
                for name in swa_snapshots[0]:
                    quad_state[name] = torch.zeros_like(swa_snapshots[0][name], dtype=torch.float32)
                    for i, snap in enumerate(swa_snapshots):
                        quad_state[name] += weights_quad[i].item() * snap[name].float()
                candidate_states.append(quad_state)
                candidate_labels.append("quad_weighted")
                
                # Candidate 6: Strided average - every 2nd snapshot
                if n_snaps >= 8:
                    for offset in range(2):
                        strided_snaps = swa_snapshots[offset::2]
                        if len(strided_snaps) >= 4:
                            s_state = {}
                            for name in swa_snapshots[0]:
                                s_state[name] = torch.zeros_like(swa_snapshots[0][name], dtype=torch.float32)
                                for snap in strided_snaps:
                                    s_state[name] += snap[name].float()
                                s_state[name] /= len(strided_snaps)
                            candidate_states.append(s_state)
                            candidate_labels.append(f"stride2_offset{offset}")
                
                # Candidate 7: Strided average - every 3rd snapshot
                if n_snaps >= 12:
                    for offset in range(3):
                        strided_snaps = swa_snapshots[offset::3]
                        if len(strided_snaps) >= 4:
                            s_state = {}
                            for name in swa_snapshots[0]:
                                s_state[name] = torch.zeros_like(swa_snapshots[0][name], dtype=torch.float32)
                                ...
                                s_state[name] /= len(strided_snaps)
                            candidate_states.append(s_state)
                            candidate_labels.append(f"stride3_offset{offset}")
                
                # Candidate 8: EMA shadow
                if ema_initialized and ema_state is not None:
                    candidate_states.append(ema_state)
                    candidate_labels.append("ema_0.998")
                
                # Evaluate each candidate
                for ci, (cand_state, cand_label) in enumerate(zip(candidate_states, candidate_labels)):
                    ...
                    cand_loss = val_loss_cand.item()
                    print0(f"  candidate '{cand_label}': val_loss={cand_loss:.6f}")
                    
                    if cand_loss < best_val_loss:
                        best_val_loss = cand_loss
            ...
            val_loss = torch.tensor(best_val_loss, device=device)
            
            # log to console and to file
            print0(f"step:{step}/{args.num_iterations} | val loss {val_loss:.6f} (best: {best_label})")
            if master_process:
                if args.log_wandb:
        ...
        if not ema_initialized:
            ema_state = {}
            for name, param in raw_model.named_parameters():
                ema_state[name] = param.data.clone().cpu()
            ema_initialized = True
        else:
            for name, param in raw_model.named_parameters():
                ema_state[name].mul_(EMA_DECAY).add_(param.data.cpu(), alpha=1.0 - EMA_DECAY)
