...
# -----------------------------------------------------------------------------
# EMA and label smoothing hyperparameters (hardcoded)
EMA_DECAY = 0.999
...
if __name__ == "__main__":
    ...
    # ----- EMA setup -----
    # We store EMA weights as a list of tensors (not requiring grad)
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            model.eval()
            # Save current training weights and load EMA weights
            training_params = []
            for p, ema_p in zip(raw_model.parameters(), ema_params):
                training_params.append(p.data.clone())
                p.data.copy_(ema_p)
            ...
            with torch.no_grad():
                ...
                for _ in range(val_steps):  # always fixed number of validation steps
                    ...
                    # For validation, use standard cross-entropy without label smoothing
                    with ctx:
                        b_val, t_val = x_val.size()
                        x_emb = raw_model.transformer.wte(x_val)
                        for block in raw_model.transformer.h:
                            x_emb = block(x_emb)
                        x_emb = rmsnorm(x_emb)
                        logits_val = raw_model.lm_head(x_emb)
                        loss = F.cross_entropy(
            ...
            for p, tp in zip(raw_model.parameters(), training_params):
                p.data.copy_(tp)
            del training_params
