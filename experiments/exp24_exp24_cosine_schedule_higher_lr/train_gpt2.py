...
if __name__ == "__main__":
    ...
    # New: cosine schedule flag
    parser.add_argument(
        "--use_cosine_schedule",
        action="store_true",
        default=False,
        help="use cosine annealing instead of linear warmdown",
    )
    parser.add_argument(
        "--min_lr_ratio",
        type=float,
        default=0.1,
        help="minimum learning rate as ratio of max lr for cosine schedule",
    )
    ...
    def get_lr(it):
        ...
        # 2) cosine annealing from peak to min_lr
        if args.use_cosine_schedule:
            progress = (it - args.warmup_iters) / max(1, args.num_iterations - args.warmup_iters)
            return min_lr + 0.5 * (args.learning_rate - min_lr) * (1.0 + math.cos(math.pi * progress))
        else:
            # Original: constant then linear warmdown
