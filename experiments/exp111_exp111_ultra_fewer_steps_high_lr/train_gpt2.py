...
MICRO_BATCH = 64
VOCAB_PADDED = 50304
INTERMEDIATE_VAL_BATCHES = 2
INTERMEDIATE_VAL_EVERY = 512
LOG_EVERY = 256
TARGET_ITERATIONS = 3800
...
if __name__ == "__main__":
    ...
    G_original = args.grad_accumulation_steps
    ...
    G_per_gpu_original = G_original // ddp_world_size
    total_tokens_per_step = B_original * G_per_gpu_original * T * ddp_world_size

    B = MICRO_BATCH
    G_new = total_tokens_per_step // (B * T * ddp_world_size)
    if G_new < 1:
        B = B_original
        G_new = G_per_gpu_original

    # Override num_iterations to TARGET_ITERATIONS
    num_iterations = min(args.num_iterations, TARGET_ITERATIONS)
    
    actual_tokens_per_step = B * G_new * T * ddp_world_size
    print0(f"Micro-batch: {B}, Grad accum per GPU: {G_new}, Tokens/step: {actual_tokens_per_step}")
    print0(f"num_iterations overridden to: {num_iterations}")
    ...
    def get_lr(it):
        ...
        if it < WARMUP:
            ...
            decay_ratio = (num_iterations - it) / WARMDOWN
    ...
    for step in range(num_iterations + 1):
        ...
        for micro_step in range(G_new):
            model.require_backward_grad_sync = (micro_step == G_new - 1)
            with ctx:
