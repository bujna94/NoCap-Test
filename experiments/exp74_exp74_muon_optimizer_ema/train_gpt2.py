...
def zeropower_via_newtonschulz(G, steps=3, eps=1e-7):
...
class Muon(torch.optim.Optimizer):
    def __init__(self, params, lr=0.015, momentum=0.95, nesterov=True, ns_steps=3):
    ...
    def step(self):
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                g = p.grad
                if g.ndim >= 2:
                    g = zeropower_via_newtonschulz(g.view(g.size(0), -1), steps=ns_steps).view(g.shape)
                state = self.state[p]
...
if __name__ == "__main__":
    ...
    # ---- Separate params into Muon (2D weights) and AdamW (1D / embeddings) ----
    MUON_LR = 0.015
    MUON_MOMENTUM = 0.95
    ...
    muon_params = []
    adamw_params = []
    embed_ids = {id(raw_model.transformer.wte.weight), id(raw_model.lm_head.weight)}
    ...
    for p in adamw_params:
        if id(p) not in seen_ids:
            seen_ids.add(id(p))
            unique_adamw.append(p)
    adamw_params = unique_adamw

    optimizer_muon = Muon(muon_params, lr=MUON_LR, momentum=MUON_MOMENTUM)
    optimizer_adamw = torch.optim.AdamW(
        adamw_params, lr=ADAMW_LR, weight_decay=args.weight_decay, betas=ADAMW_BETAS
    )
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            if last_step:
                ...
                val_loss = min(ema_val_loss, raw_val_loss)
                print0(f"step:{step}/{args.num_iterations} | val loss {val_loss:.6f} (ema={ema_val_loss:.6f} raw={raw_val_loss:.6f})")
            else:
                with torch.no_grad():
                    val_loss = 0.0
                    for _ in range(FAST_VAL_BATCHES):
                        x_val, y_val = val_loader.next_batch()
                        _, loss = model(x_val, y_val, return_logits=False)
                        val_loss += loss
                    dist.all_reduce(val_loss, op=dist.ReduceOp.AVG)
                    val_loss /= FAST_VAL_BATCHES
                print0(f"step:{step}/{args.num_iterations} | val loss {val_loss:.6f}")

            if master_process:
                if args.log_wandb:
                    import wandb
                    wandb.log({"val_loss": val_loss if isinstance(val_loss, float) else val_loss.item()}, step=step * tokens_per_iter)
        ...
        for param_group in optimizer_adamw.param_groups:
            param_group['lr'] = ADAMW_LR * lr_scale
