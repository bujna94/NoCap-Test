...
if __name__ == "__main__":
    ...
    print0(f"grad_accumulation_steps per GPU: {args.grad_accumulation_steps}")
    ...
    def is_warmdown_phase(it):
        return it >= args.num_iterations - WARMDOWN_STEPS
    ...
    for step in range(args.num_iterations + 1):
        ...
        # Validation logic: full at step 0 and last step, skip all intermediate
        do_val = False
        if last_step:
            do_val = True
        elif args.val_loss_every > 0 and step % args.val_loss_every == 0 and step == 0:
            do_val = True
        ...
        if do_val:
            ...
            if last_step:
                ...
                for orig_p, p in zip(original_params, raw_model.parameters()):
                    p.data = orig_p
            ...
            if master_process and logfile is not None:
                with open(logfile, "a") as f:
                    f.write("s:%d val:%f\n" % (step, val_loss if not isinstance(val_loss, float) else val_loss))
        ...
        if step >= WARMUP_STEPS:
            if is_warmdown_phase(step):
                update_ema(EMA_DECAY_WARMDOWN)
            else:
                update_ema(EMA_DECAY_TRAIN)
