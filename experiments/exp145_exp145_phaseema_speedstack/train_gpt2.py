...
class GPT(nn.Module):
    ...
    def configure_optimizers(self, weight_decay, learning_rate, betas, device_type):
        optimizer = torch.optim.AdamW(
            self.parameters(), lr=learning_rate, weight_decay=weight_decay, betas=betas, fused=True
        )
        return optimizer
...
if __name__ == "__main__":
    ...
    # Linear warmdown LR schedule (proven in exp127)
    def get_lr(it):
        assert it <= args.num_iterations
        if it < args.warmup_iters:
            return CUSTOM_LR * (it + 1) / args.warmup_iters
        elif it < args.num_iterations - CUSTOM_WARMDOWN:
            return CUSTOM_LR
        else:
            decay_ratio = (args.num_iterations - it) / CUSTOM_WARMDOWN
            return CUSTOM_LR * decay_ratio
    ...
    for step in range(args.num_iterations + 1):
        ...
        # Validation: only at step 0 and final step for speed
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            torch.cuda.synchronize()
            training_time_ms += 1000 * (time.perf_counter() - t0)

            if last_step:
                # Full dual evaluation at final step
                model.eval()
                val_loader.reset()
                with torch.no_grad():
                    val_loss_raw = 0.0
                ...
                orig_data = [p.data.clone() for p in raw_params]
                ...
                print0(f"step:{step}/{args.num_iterations} | val_raw:{val_loss_raw:.6f} | val_ema:{val_loss_ema:.6f} | best:{val_loss:.6f}")
            else:
                # Non-final: skip intermediate validations entirely for speed
                # Only log step 0 with a quick 1-batch estimate
                if step == 0:
                    model.eval()
                    val_loader.reset()
                    with torch.no_grad():
                        x_val, y_val = val_loader.next_batch()
                        _, val_loss = model(x_val, y_val, return_logits=False)
                        dist.all_reduce(val_loss, op=dist.ReduceOp.AVG)
                    print0(f"step:{step}/{args.num_iterations} | val_loss:{val_loss:.6f} (1-batch approx)")
                else:
                    val_loss = float('nan')

            if master_process and logfile is not None and not math.isnan(float(val_loss)):
        ...
        warmdown_start = args.num_iterations - CUSTOM_WARMDOWN
