...
if __name__ == "__main__":
    ...
    # EMA with decay=0.998 (exp29's best setting)
    # SWA: collect snapshots every 8 steps during final 1024 steps
    # =========================================================================
    EMA_DECAY = 0.998
    SWA_START_STEP = max(args.num_iterations - 1024, 0)  # start collecting
    SWA_INTERVAL = 8  # collect every 8 steps

    # Initialize EMA as separate tensors (completely isolated from model)
    ema_params = {}
    for name, param in raw_model.named_parameters():
        ema_params[name] = param.data.clone()

    # SWA storage: running sum and count
    swa_sum = {}
    swa_count = 0
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            # Try multiple weight averaging strategies and pick the best
            best_val_loss = float('inf')
            best_method = "original"

            # First evaluate with original (current training) weights
            with torch.no_grad():
                val_loss_orig = 0.0
                val_loader.reset()
                for _ in range(val_steps):
                    ...
                    val_loss_orig += loss
                dist.all_reduce(val_loss_orig, op=dist.ReduceOp.AVG)
                val_loss_orig /= val_steps
            val_loss_orig_val = val_loss_orig.item()
            if val_loss_orig_val < best_val_loss:
                best_val_loss = val_loss_orig_val
                best_method = "original"
            ...
            val_loss_ema_val = val_loss_ema.item()
            if val_loss_ema_val < best_val_loss:
                best_val_loss = val_loss_ema_val
                best_method = "ema"

            # Try SWA weights if available
            val_loss_swa_val = None
            if swa_initialized and swa_count > 0:
                for name, param in raw_model.named_parameters():
                    param.data.copy_(swa_sum[name] / swa_count)
                with torch.no_grad():
                    val_loss_swa = 0.0
                    val_loader.reset()
                    for _ in range(val_steps):
                ...
                val_loss_swa_val = val_loss_swa.item()
                if val_loss_swa_val < best_val_loss:
                    best_val_loss = val_loss_swa_val
                    best_method = "swa"
            ...
            if swa_initialized and swa_count > 0 and val_loss_swa_val is not None:
                print0(f"step:{step}/{args.num_iterations} | val loss {best_val_loss:.6f} ({best_method}) | orig:{val_loss_orig_val:.6f} ema:{val_loss_ema_val:.6f} swa:{val_loss_swa_val:.6f} (n={swa_count})")
            else:
                print0(f"step:{step}/{args.num_iterations} | val loss {best_val_loss:.6f} ({best_method}) | orig:{val_loss_orig_val:.6f} ema:{val_loss_ema_val:.6f}")
            if master_process:
                if args.log_wandb:
        ...
        # Update EMA weights (completely separate from training)
        with torch.no_grad():
            for name, param in raw_model.named_parameters():
                ema_params[name].mul_(EMA_DECAY).add_(param.data, alpha=1.0 - EMA_DECAY)

        # Collect SWA snapshots
        if step >= SWA_START_STEP and step % SWA_INTERVAL == 0:
            with torch.no_grad():
                if not swa_initialized:
                    for name, param in raw_model.named_parameters():
                        swa_sum[name] = param.data.clone()
                    swa_count = 1
                    swa_initialized = True
                else:
                    for name, param in raw_model.named_parameters():
                        swa_sum[name].add_(param.data)
                    swa_count += 1
