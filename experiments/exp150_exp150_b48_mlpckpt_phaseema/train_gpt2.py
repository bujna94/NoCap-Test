...
WARMUP_STEPS = 256
WARMDOWN_STEPS = 1024
EMA_DECAY_BASE = 0.998
EMA_DECAY_LATE = 0.9995
EMA_LATE_START_FRAC = 0.75
MICRO_BATCH = 48
...
class Block(nn.Module):
    def __init__(self, cfg):
        ...
        self.mlp = MLP(cfg)
        self.attn_scale = 1 / math.sqrt(2 * cfg.n_layer)

    def forward(self, x):
        x = x + self.attn_scale * self.attn(rmsnorm(x))
        x = x + checkpoint(self._mlp_forward, rmsnorm(x), use_reentrant=False)
        return x
...
class GPT(nn.Module):
    def __init__(self, cfg):
        ...
        self.lm_head = nn.Linear(cfg.vocab_size, cfg.n_embd, bias=False)
...
if __name__ == "__main__":
    ...
    # Robust grad_accum calculation
    tokens_per_micro = B * T
    grad_accum_per_gpu = max(1, TOTAL_BATCH_TOKENS // (tokens_per_micro * ddp_world_size))
    tokens_per_iter = tokens_per_micro * grad_accum_per_gpu * ddp_world_size
    print0(f"B={B}, grad_accum_per_gpu={grad_accum_per_gpu}, tokens_per_iter={tokens_per_iter:,}")
    ...
    def get_ema_decay(it):
        if it >= args.num_iterations - WARMDOWN_STEPS:
            return EMA_DECAY_LATE
        elif it >= int(args.num_iterations * EMA_LATE_START_FRAC):
            return 0.999
        return EMA_DECAY_BASE
    ...
    for step in range(args.num_iterations + 1):
        ...
        if last_step or step == 0 or (args.val_loss_every > 0 and step % args.val_loss_every == 0):
            torch.cuda.synchronize()
            ...
            if last_step:
                ...
                print0(f"step:{step}/{args.num_iterations} val_raw:{val_loss_raw:.6f} val_ema:{val_loss_ema:.6f} best:{val_loss:.6f} train_time:{training_time_ms/1000:.2f}s")
            else:
                with torch.no_grad():
                    val_loss = 0.0
                    for _ in range(val_steps):
                    ...
                    val_loss /= val_steps
                print0(f"step:{step}/{args.num_iterations} val:{val_loss:.6f} train_time:{training_time_ms/1000:.2f}s")
        ...
        if step % LOG_EVERY == 0 or step < 2:
            ...
            print0(f"step:{step}/{args.num_iterations} loss:{train_loss.item():.6f} time:{approx_time/1000:.2f}s ema_d:{ema_decay:.4f}")
            if master_process and logfile is not None:
                with open(logfile, "a") as f:
