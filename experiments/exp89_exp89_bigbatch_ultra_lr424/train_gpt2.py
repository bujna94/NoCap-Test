...
if __name__ == "__main__":
    ...
    def get_lr(it):
        ...
        if it < CUSTOM_WARMUP:
            return CUSTOM_LR * (it + 1) / CUSTOM_WARMUP
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            if last_step:
                ...
                ema_val = val_loss.item() if isinstance(val_loss, torch.Tensor) else val_loss
                raw_val = raw_val_loss.item() if isinstance(raw_val_loss, torch.Tensor) else raw_val_loss
                final_val_loss = min(ema_val, raw_val)
                print0(f"step:{step}/{args.num_iterations} | val loss {final_val_loss:.6f} (ema={ema_val:.6f}, raw={raw_val:.6f})")
                ...
                if master_process and logfile is not None:
                    with open(logfile, "a") as f:
                        f.write("s:%d val:%f\n" % (step, val_loss.item() if isinstance(val_loss, torch.Tensor) else val_loss))
        ...
        for ema_p, p in zip(ema_params, param_list):
