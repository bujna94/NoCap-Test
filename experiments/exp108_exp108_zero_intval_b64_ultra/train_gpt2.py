...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, return_logits=True):
        ...
        if targets is not None:
            ...
            loss = F.cross_entropy(logits_for_loss, targets_flat, ignore_index=-1)
        else:
...
if __name__ == "__main__":
    ...
    # Maintain same total tokens per step as baseline
    # Harness passes: batch_size * grad_accumulation_steps (already divided by world_size for grad_accum)
    # So total tokens/step = batch_size * grad_accumulation_steps * T * world_size
    # We want: B * new_grad_accum * T * world_size = same
    # => new_grad_accum = (batch_size * grad_accumulation_steps) / B
    # But grad_accumulation_steps from harness is already per-GPU
    # So: new_grad_accum_per_gpu = (args.batch_size * args.grad_accumulation_steps) // B
    grad_accum_per_gpu = (args.batch_size * args.grad_accumulation_steps) // B
    if grad_accum_per_gpu < 1:
        grad_accum_per_gpu = 1

    tokens_per_iter = B * T * ddp_world_size * grad_accum_per_gpu
    print0(f"tokens per iteration: {tokens_per_iter:,} (B={B}, T={T}, grad_accum={grad_accum_per_gpu}, world={ddp_world_size})")
    ...
    for step in range(args.num_iterations + 1):
        ...
        is_val_step = (step == 0 or last_step) and val_loss_every > 0
        ...
        if is_val_step:
            ...
            if use_ema:
                ...
                for i, p in enumerate(model_params):
                    ema_params[i], p.data = p.data, ema_params[i]
            ...
            label = "(EMA)" if use_ema else ""
            print0(f"step:{step}/{args.num_iterations} | val loss {label} {val_loss:.6f}")
        ...
        # Sparse logging
        if step % 256 == 0 or step < 3:
            torch.cuda.synchronize()
            approx_time = training_time_ms + 1000 * (time.perf_counter() - t0)
            print0(f"step:{step}/{args.num_iterations} | lr {lr:.6f} | train_time:{approx_time/1000:.2f}s | step_avg:{approx_time/(step+1):.2f}ms")
