...
if __name__ == "__main__":
    ...
    # SWA: collect snapshots every 8 steps over final 1024 steps
    SWA_START_STEP = max(args.num_iterations - 1024, 0)  # handle small num_iterations
    SWA_EVERY = 8
    swa_snapshots = []  # list of state_dict copies (CPU tensors to save GPU memory)
    
    # EMA: maintain 3 shadows with different decays
    EMA_DECAYS = [0.998, 0.999, 0.9995]
    ema_shadows = []  # list of dicts, each mapping param_name -> tensor
    ema_initialized = False

    def init_ema_shadows(raw_model):
        shadows = []
    ...
    def update_ema_shadows(raw_model, shadows):
        for i, decay in enumerate(EMA_DECAYS):
            for name, param in raw_model.named_parameters():
                shadows[i][name].mul_(decay).add_(param.data, alpha=1.0 - decay)

    def compute_swa_average(snapshots, weights=None):
        """Compute weighted average of snapshots. If weights is None, uniform average."""
        if weights is None:
        ...
        for key in snapshots[0]:
            avg[key] = torch.zeros_like(snapshots[0][key], dtype=torch.float32)
            for i, snap in enumerate(snapshots):
                avg[key].add_(snap[key].float(), alpha=weights[i])
            avg[key] = avg[key].to(snapshots[0][key].dtype)
        return avg

    def evaluate_candidate(model, raw_model, candidate_sd, val_loader, val_steps, device):
        """Evaluate a candidate state dict on validation set."""
        # Save current weights
        orig_sd = {name: param.data.clone() for name, param in raw_model.named_parameters()}
        # Load candidate
        for name, param in raw_model.named_parameters():
            if name in candidate_sd:
                param.data.copy_(candidate_sd[name])
        # Evaluate
        model.eval()
        val_loader.reset()
        with torch.no_grad():
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            # For the last step (or any step where we have candidates), evaluate all candidates
            if last_step and len(swa_snapshots) > 0:
                # Build list of candidates
                candidates = []
                candidate_names = []
                
                # 1) Current model weights (baseline)
                current_sd = {name: param.data.clone() for name, param in raw_model.named_parameters()}
                candidates.append(current_sd)
                ...
                # 3) Uniform SWA averages over different tail windows
                n_snaps = len(swa_snapshots)
                for tail_steps in [64, 128, 256, 512, 768, 1024]:
                    # How many snapshots correspond to this many steps?
                    n_tail = tail_steps // SWA_EVERY
                    if n_tail >= 2 and n_tail <= n_snaps:
                        tail_snaps = swa_snapshots[-n_tail:]
                        avg = compute_swa_average(tail_snaps)
                        candidates.append(avg)
                        candidate_names.append(f"swa_uniform_{tail_steps}")
                
                # 4) Linearly-weighted SWA (more weight to later snapshots)
                for tail_steps in [512, 1024]:
                    n_tail = tail_steps // SWA_EVERY
                    if n_tail >= 2 and n_tail <= n_snaps:
                        tail_snaps = swa_snapshots[-n_tail:]
                        weights = [float(i + 1) for i in range(len(tail_snaps))]
                        avg = compute_swa_average(tail_snaps, weights)
                        candidates.append(avg)
                        candidate_names.append(f"swa_linear_{tail_steps}")
                
                # 5) Quadratically-weighted SWA
                for tail_steps in [512, 1024]:
                    n_tail = tail_steps // SWA_EVERY
                    if n_tail >= 2 and n_tail <= n_snaps:
                        tail_snaps = swa_snapshots[-n_tail:]
                        weights = [float((i + 1) ** 2) for i in range(len(tail_snaps))]
                        avg = compute_swa_average(tail_snaps, weights)
                        candidates.append(avg)
                        candidate_names.append(f"swa_quad_{tail_steps}")
                ...
                for cand_sd, cand_name in zip(candidates, candidate_names):
                    cand_loss = evaluate_candidate(model, raw_model, cand_sd, val_loader, val_steps, device)
                    print0(f"  Candidate '{cand_name}': val_loss = {cand_loss:.6f}")
                ...
                # Restore current weights for any further operations
                for name, param in raw_model.named_parameters():
                    param.data.copy_(current_sd[name])
            else:
                # Standard evaluation for non-last steps
                model.eval()
                val_loader.reset()
                with torch.no_grad():
                    val_loss = 0.0
