...
VAL_EVERY_OVERRIDE = 340
LOG_EVERY = 200
NUM_ITERS_OVERRIDE = 3400
...
if __name__ == "__main__":
    ...
    # Override num_iterations for ULTRA attempt
    args.num_iterations = NUM_ITERS_OVERRIDE
    ...
    tokens_per_step_per_gpu = args.batch_size * orig_grad_accum_per_gpu * T
    effective_grad_accum = max(1, tokens_per_step_per_gpu // (B * T))
    actual_tokens_per_step_per_gpu = effective_grad_accum * B * T

    device = f"cuda:{ddp_local_rank}"
    torch.cuda.set_device(device)
    master_process = ddp_rank == 0
    print0(f"using device: {device}")
    print0(f"Micro-batch: {B}, grad_accum per GPU: {effective_grad_accum}, tokens/step/gpu: {actual_tokens_per_step_per_gpu}")
    print0(f"NUM ITERATIONS OVERRIDE: {args.num_iterations}")

    torch.set_float32_matmul_precision('high')

    tokens_per_iter = actual_tokens_per_step_per_gpu * ddp_world_size
    ...
    num_vocab = PADDED_VOCAB if USE_VOCAB_PAD else 50257
    model_config = {
    ...
    def get_lr(it):
        ...
        if it < EXP_WARMUP:
            return EXP_LR * (it + 1) / EXP_WARMUP
        elif it < args.num_iterations - EXP_WARMDOWN:
            return EXP_LR
        else:
            decay_ratio = (args.num_iterations - it) / EXP_WARMDOWN
            return EXP_LR * decay_ratio
    ...
    for step in range(args.num_iterations + 1):
        ...
        do_val = val_loss_every > 0 and (step % val_loss_every == 0 or last_step)
        if do_val:
            torch.cuda.synchronize()
            ...
            if last_step:
                ...
                n_val = INTERMEDIATE_VAL_BATCHES
                with torch.no_grad():
                    val_loss = 0.0
                    for _ in range(n_val):
                        x_val, y_val = val_loader.next_batch()
