...
if __name__ == "__main__":
    ...
    param_list = list(raw_model.parameters())
    ema_params = [p.data.clone() for p in param_list]
    swa_sum = None
    swa_count = 0

    # Identify parameters for gradient centralization
    gc_param_set = set()
    for name, p in raw_model.named_parameters():
        if p.ndim >= 2 and 'wte' not in name:
            gc_param_set.add(id(p))
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            if last_step:
                ...
                # Try EMA weights
                orig_params = [p.data.clone() for p in param_list]
                for p, ema_p in zip(param_list, ema_params):
                    p.data.copy_(ema_p)
                ...
                if swa_sum is not None and swa_count > 0:
                    for p, s in zip(param_list, swa_sum):
                ...
                for p, orig_p in zip(param_list, orig_params):
                    p.data.copy_(orig_p)
                del orig_params
            ...
            if master_process:
                if args.log_wandb:
                    vl = best_val_loss if last_step else val_loss.item()
        ...
        # Apply Gradient Centralization to weight matrices
        for p in param_list:
            if p.grad is not None and id(p) in gc_param_set:
                if p.grad.ndim >= 2:
                    p.grad.data.sub_(p.grad.data.mean(dim=tuple(range(1, p.grad.ndim)), keepdim=True))
        ...
        # Update SWA running sum (uses EMA params for better averaging)
        if step >= SWA_START_STEP and (step - SWA_START_STEP) % SWA_FREQ == 0:
            with torch.no_grad():
                if swa_sum is None:
                    swa_sum = [ep.clone() for ep in ema_params]
                    swa_count = 1
                else:
                    for s, ep in zip(swa_sum, ema_params):
                        s.add_(ep)
                    swa_count += 1
