...
def select_micro_batch_size(original_B, original_grad_accum):
    """Select largest micro-batch that divides total evenly."""
    total = original_B * original_grad_accum
    # Try candidates from large to small
    for candidate_B in [64, 48, 32, 16]:
        if total % candidate_B == 0:
            return candidate_B, total // candidate_B
    return original_B, original_grad_accum
...
if __name__ == "__main__":
    ...
    # Select optimal micro-batch size (B=64 proven safe, try it as primary)
    effective_B, effective_grad_accum = select_micro_batch_size(B, args.grad_accumulation_steps)
    print0(f"Using micro-batch B={effective_B}, grad_accum={effective_grad_accum} (original B={B}, grad_accum={args.grad_accumulation_steps})")
    print0(f"Total samples/step/GPU: {effective_B * effective_grad_accum} (was {B * args.grad_accumulation_steps})")

    # Quality hyperparameters (proven in exp72/85/105)
    PEAK_LR = 0.003
    WARMUP_ITERS = 256
    WARMDOWN_ITERS = 1024
    EMA_DECAY = 0.998

    # Speed: reduce intermediate validation
    INTERMEDIATE_VAL_BATCHES = 4
    LOG_EVERY = 128

    tokens_per_iter = effective_B * T * ddp_world_size * effective_grad_accum
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            if last_step:
                ...
                # Swap in EMA weights for final eval
                ...
                for p, orig in zip(model_params, original_data):
                    p.data = orig
            else:
