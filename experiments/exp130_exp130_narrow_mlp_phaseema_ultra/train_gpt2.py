...
MLP_HIDDEN = 2688  # 3.5x expansion, divisible by 128
...
if __name__ == "__main__":
    ...
    for step in range(args.num_iterations + 1):
        ...
        if do_val:
            ...
            model.eval()
            val_loader.reset()
            current_val_steps = val_steps if last_step else VAL_BATCHES_INTERMEDIATE
            with torch.no_grad():
                val_loss = 0.0
        ...
        if step >= WARMUP_ITERS:
            if not ema_started:
                ...
                if step < args.num_iterations - WARMDOWN_ITERS:
