...
# ---------- HARDCODED SPEEDUP PARAMS ----------
MICRO_BATCH = 64          # larger micro-batch for GPU throughput
PEAK_LR = 0.003           # proven optimal LR from exp72/85/127
WARMUP_STEPS = 256        # standard warmup
WARMDOWN_STEPS = 1024     # standard warmdown
EMA_DECAY_TRAIN = 0.998   # EMA during constant LR phase
EMA_DECAY_WARMDOWN = 0.9995  # EMA during warmdown phase (phase-aware)
GRAD_CLIP_NORM = 1.0      # gradient clipping for B=64 stability
SPARSE_LOG_EVERY = 128    # only log train loss every N steps
INTERMEDIATE_VAL_BATCHES = 4  # reduced intermediate validation
VOCAB_SIZE_PADDED = 50304 # pad vocab for tensor core alignment
# -----------------------------------------------
...
if __name__ == "__main__":
    ...
    B = MICRO_BATCH
    total_grad_accum_orig = args.grad_accumulation_steps
    total_tokens_per_step = B_orig * total_grad_accum_orig * T
    total_grad_accum_new = max(1, total_tokens_per_step // (B * T))

    if total_grad_accum_new % ddp_world_size != 0:
        total_grad_accum_new = max(1, (total_grad_accum_new // ddp_world_size) * ddp_world_size)
        if total_grad_accum_new == 0:
            total_grad_accum_new = ddp_world_size

    grad_accumulation_steps = total_grad_accum_new // ddp_world_size
    if grad_accumulation_steps < 1:
        grad_accumulation_steps = 1

    tokens_per_iter = B * T * ddp_world_size * grad_accumulation_steps
    print0(f"tokens per iteration: {tokens_per_iter:,} (B={B}, grad_accum_per_gpu={grad_accumulation_steps})")
    ...
    if VAL_TOKENS % tokens_per_iter_val != 0:
        val_steps = max(1, VAL_TOKENS // tokens_per_iter_val)
    else:
    ...
    warmdown_steps = min(WARMDOWN_STEPS, num_iterations)
    ...
    # Initialize EMA shadows AFTER model is on GPU and compiled
    ...
    for step in range(num_iterations + 1):
        ...
        if val_loss_every > 0 and (step % val_loss_every == 0 or last_step):
            ...
            if is_final:
                ...
                for p, ema_p in zip(raw_model.parameters(), ema_params):
                    raw_params_backup.append(p.data.clone())
                    p.data.copy_(ema_p)
                ...
                for p, raw_p in zip(raw_model.parameters(), raw_params_backup):
                    p.data.copy_(raw_p)

                val_loss = min(val_loss_raw.item(), val_loss_ema.item())
                print0(f"step:{step}/{num_iterations} | val loss {val_loss:.6f} (raw={val_loss_raw.item():.6f}, ema={val_loss_ema.item():.6f})")
            else:
                with torch.no_grad():
                    val_loss = 0.0
                    for _ in range(current_val_steps):
            ...
            if master_process and logfile is not None:
                vl = val_loss if isinstance(val_loss, float) else val_loss.item()
        ...
        if step >= warmup_steps:
            ema_decay = EMA_DECAY_WARMDOWN if step >= warmdown_start else EMA_DECAY_TRAIN
            with torch.no_grad():
        ...
        if step % SPARSE_LOG_EVERY == 0 or step < 2:
            ...
            print0(f"step:{step}/{num_iterations} | loss {lossf:.6f} | train_time:{approx_training_time_ms/1000:.2f}s | step_avg:{approx_training_time_ms/(step+1):.2f}ms")
            if master_process and logfile is not None:
                with open(logfile, "a") as f:
                    f.write("s:%d trn:%f\n" % (step, lossf))
