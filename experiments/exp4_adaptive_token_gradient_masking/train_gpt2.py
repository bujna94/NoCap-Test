...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, return_logits=True, token_mask=None):
        b, t = idx.size()
        ...
        if targets is not None:
            ...
            if token_mask is not None:
                # token_mask: (B, T) where 1 = keep, 0 = mask
                # We modify the loss computation to only use unmasked tokens
                ...
                loss = loss.view(b, t)
                # Apply mask and normalize by number of kept tokens
                loss = (loss * token_mask).sum() / token_mask.sum().clamp(min=1.0)
            else:
                loss = F.cross_entropy(
...
if __name__ == "__main__":
    ...
    # Initialize gradient statistics tracking
    grad_ema = None
    ema_alpha = 0.9
    mask_percentile = 20  # Mask bottom 20% of tokens by gradient magnitude
    ...
    for step in range(args.num_iterations + 1):
        ...
        for micro_step in range(args.grad_accumulation_steps):
            ...
            # Compute token mask based on gradient statistics (after warmup)
            token_mask = None
            if step > 100:  # Start masking after 100 steps
                with torch.no_grad():
                    # Forward pass to get per-token losses
                    with ctx:
                        logits, _ = model(x, y, return_logits=True, token_mask=None)
                    ...
                    # Use per-token loss as proxy for gradient magnitude
                    # (tokens with higher loss typically have larger gradients)
                    token_importance = per_token_loss.detach()
                    
                    # Update EMA of token importance
                    if grad_ema is None:
                        grad_ema = token_importance.mean()
                    else:
                        grad_ema = ema_alpha * grad_ema + (1 - ema_alpha) * token_importance.mean()
                    
                    # Compute threshold for masking (bottom percentile)
                    threshold = torch.quantile(token_importance.flatten(), mask_percentile / 100.0)
                    
                    # Create mask: 1 for keep, 0 for mask
                    token_mask = (token_importance >= threshold).float()
                    
                    # Ensure at least 50% of tokens are kept
                    if token_mask.sum() < (B * T * 0.5):
                        token_mask = torch.ones_like(token_mask)
            
            # Forward pass with token masking
            with ctx:
                _, loss = model(x, y, return_logits=False, token_mask=token_mask)
