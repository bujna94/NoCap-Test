...
SWA_START_STEP = 3744  # Start averaging in the last 1024 steps
SWA_UPDATE_FREQ = 64  # Update SWA weights every 64 steps
SWA_DECAY = 0.0  # 0.0 means equal averaging (not exponential)
...
if __name__ == "__main__":
    ...
    # learning rate decay scheduler (cosine warmdown instead of linear)
    def get_lr(it):
        assert it <= args.num_iterations
        # 1) linear warmup for warmup_iters steps
        if it < args.warmup_iters:
            return args.learning_rate * (it + 1) / args.warmup_iters
        # 2) cosine decay from warmup_iters to num_iterations
        # This replaces the constant + linear warmdown with a smooth cosine decay
        decay_steps = args.num_iterations - args.warmup_iters
        if decay_steps == 0:
            return args.learning_rate
        progress = (it - args.warmup_iters) / decay_steps  # 0.0 to 1.0
        # Cosine decay to a small minimum LR (not zero, to keep learning)
        min_lr_ratio = 0.0
        return args.learning_rate * (min_lr_ratio + (1.0 - min_lr_ratio) * 0.5 * (1.0 + math.cos(math.pi * progress)))
    ...
    # SWA: Initialize shadow weights as None; will be created lazily
    swa_state = None
    swa_count = 0
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            use_swa = (swa_state is not None and swa_count > 0)
            
            if use_swa:
                # Save current weights and swap in SWA weights
                saved_state = {}
            ...
            print0(f"step:{step}/{args.num_iterations} | val loss {val_loss:.6f} | swa:{use_swa}")
            if master_process:
                if args.log_wandb:
        ...
        # SWA: Update shadow weights periodically in the final phase
        if step >= SWA_START_STEP and step % SWA_UPDATE_FREQ == 0:
            if swa_state is None:
                # Initialize SWA state as a copy of current weights
                swa_state = {}
                for name, param in raw_model.named_parameters():
                ...
                for name, param in raw_model.named_parameters():
                    swa_state[name].add_((param.data - swa_state[name]) / swa_count)
