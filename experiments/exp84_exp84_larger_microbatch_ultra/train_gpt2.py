...
if __name__ == "__main__":
    ...
    # Keep original B and grad_accum - the speed gains come from other optimizations
    # Using standard B to avoid OOM risk
    effective_B = B
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            if is_final:
                ...
                orig_params = [p.data.clone() for p in model_params]
            ...
            if is_final:
                ...
                # Restore original weights and evaluate
                for i, p in enumerate(model_params):
                    p.data.copy_(orig_params[i])
                val_loader.reset()
                with torch.no_grad():
                    val_loss_raw = 0.0
                ...
                val_loss = min(ema_val_loss, raw_val)
                print0(f"EMA val: {ema_val_loss:.6f}, Raw val: {raw_val:.6f}, Using: {val_loss:.6f}")
            else:
        ...
        for micro_step in range(effective_grad_accum):
            model.require_backward_grad_sync = (
                micro_step == effective_grad_accum - 1
            )
            with ctx:
