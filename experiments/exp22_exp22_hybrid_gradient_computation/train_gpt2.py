...
class GradientVarianceTracker:
    ...
    def update(self, name, grad):
        ...
        grad_norm_value = grad_flat.norm() + 1e-8
        grad_norm = grad_flat / grad_norm_value
        
        if name not in self.grad_history:
            self.grad_history[name] = deque(maxlen=self.window_size)
        
        self.grad_history[name].append(grad_norm.cpu())
        
        if len(self.grad_history[name]) < 2:
            return 1.0
        
        cosines = []
        grads = list(self.grad_history[name])
        for i in range(len(grads) - 1):
            cos_sim = torch.dot(grads[i], grads[i+1]).item()
            cosines.append(cos_sim)
        
        variance = np.var(cosines) if len(cosines) > 0 else 1.0
        return variance
...
if __name__ == "__main__":
    ...
    grad_tracker = GradientVarianceTracker(window_size=10)
        ...
        for step in range(args.num_iterations + 1):
            ...
            for name, param in raw_model.named_parameters():
                if param.grad is not None:
                    variance = grad_tracker.update(name, param.grad)
