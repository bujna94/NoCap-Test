...
class Block(nn.Module):
    ...
    def get_attn_scale(self):
        """
        Compute depth-dependent attention scale that evolves during training.
        """
        # Depth factor: shallow layers (small layer_idx) get higher initial scale
        # Layer 0 gets 2x, layer n_layer-1 gets 1x the base scale initially
        depth_ratio = self.layer_idx / (self.n_layer - 1) if self.n_layer > 1 else 0.5
        initial_multiplier = 2.0 - depth_ratio  # ranges from 2.0 (layer 0) to 1.0 (last layer)
        
        # Schedule: interpolate from depth-dependent to uniform over first 50% of training
        schedule_progress = min(self.training_progress / 0.5, 1.0)
        current_multiplier = initial_multiplier * (1 - schedule_progress) + 1.0 * schedule_progress
        
        return self.base_attn_scale * current_multiplier

    def forward(self, x):
        attn_scale = self.get_attn_scale()
        x = x + attn_scale * self.attn(rmsnorm(x))
...
class GPT(nn.Module):
    ...
    def set_training_progress(self, progress):
        """Update training progress for all blocks"""
        for block in self.transformer.h:
            block.training_progress = progress
...
if __name__ == "__main__":
    ...
    for step in range(args.num_iterations + 1):
        ...
        # Update training progress for attention scale scheduling
        training_progress = step / max(args.num_iterations, 1)
        raw_model.set_training_progress(training_progress)
