...
class EMAModel:
    ...
    def __init__(self, model, decay=0.995):
        self.decay = decay
        # Store EMA parameters as a flat dict of tensors (not a model copy to save compile overhead)
        self.shadow = {}
        self.backup = {}
    ...
    def apply_shadow(self, model):
        """Replace model params with EMA params, backing up originals."""
        for name, param in model.named_parameters():
            if param.requires_grad and name in self.shadow:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.993,
        help="EMA decay rate for weight averaging",
    )
    ...
    # Initialize EMA after model is on GPU but before training
    # We use the raw (unwrapped from DDP but still compiled) model's named_parameters
    ema = EMAModel(raw_model, decay=args.ema_decay)
    print0(f"EMA initialized with decay={args.ema_decay}")
