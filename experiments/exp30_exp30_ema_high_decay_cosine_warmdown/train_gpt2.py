...
# EMA configuration - hardcoded
EMA_DECAY = 0.9999  # Very high decay to avoid over-smoothing (exp29 showed 0.998 was too aggressive)
EMA_START_STEP = 3000  # Only start EMA after model has converged significantly
...
if __name__ == "__main__":
    ...
    # learning rate decay scheduler: linear warmup + cosine warmdown (instead of linear warmdown)
    def get_lr(it):
        ...
        # 3) cosine warmdown (instead of linear) - spends more time at moderate LRs
        else:
            progress = (args.num_iterations - it) / args.warmdown_iters  # 1.0 -> 0.0
            # Cosine shape: slower initial decay, then faster at the end
            return args.learning_rate * 0.5 * (1.0 + math.cos(math.pi * (1.0 - progress)))
    ...
    def init_ema():
        ...
        print0(f"EMA initialized with {len(ema_params)} parameter tensors")
    ...
    def swap_ema_in():
        """Swap EMA weights into model, save originals"""
        originals = {}
        with torch.no_grad():
            for name, param in raw_model.named_parameters():
                if name in ema_params:
    ...
    def swap_ema_out(originals):
        """Restore original weights from saved originals"""
        with torch.no_grad():
            for name, param in raw_model.named_parameters():
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            # Determine if we should use EMA weights for validation
            use_ema = ema_initialized and step >= EMA_START_STEP
            originals = None
            if use_ema:
                originals = swap_ema_in()
            ...
            # Swap back original weights if we used EMA
            if use_ema and originals is not None:
                swap_ema_out(originals)

            # log to console and to file
            ema_tag = " (EMA)" if use_ema else ""
            print0(f"step:{step}/{args.num_iterations} | val loss {val_loss:.6f}{ema_tag}")
            if master_process:
                if args.log_wandb:
