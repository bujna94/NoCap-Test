...
if __name__ == "__main__":
    ...
    # Dynamic gradient accumulation variables
    prev_grad_vec = None
    current_accum_steps = args.grad_accumulation_steps
    min_accum_steps = 1
    max_accum_steps = 4
    grad_similarity_ema = 0.8  # initial high similarity assumption
    ema_alpha = 0.3
    ...
    for step in range(args.num_iterations + 1):
        ...
        # Dynamically adjust accumulation steps based on gradient similarity
        effective_accum_steps = current_accum_steps
        
        for micro_step in range(effective_accum_steps):
            model.require_backward_grad_sync = (
                micro_step == effective_accum_steps - 1
            )  # sync only on last micro step to avoid overhead
            # forward pass
            with ctx:
                _, loss = model(x, y, return_logits=False)
                loss = (
                    loss / effective_accum_steps
        ...
        # Measure gradient direction for adaptive accumulation
        if step > 0 and step % 10 == 0:  # Check every 10 steps to reduce overhead
            with torch.no_grad():
                # Flatten all gradients into a single vector
                grad_vec = torch.cat([p.grad.flatten() for p in model.parameters() if p.grad is not None])
                grad_norm = torch.norm(grad_vec)
                
                if prev_grad_vec is not None and grad_norm > 1e-8:
                    # Compute cosine similarity between current and previous gradient
                    prev_norm = torch.norm(prev_grad_vec)
                    if prev_norm > 1e-8:
                        cosine_sim = torch.dot(grad_vec, prev_grad_vec) / (grad_norm * prev_norm)
                        cosine_sim = cosine_sim.item()
                        
                        # Update EMA of gradient similarity
                        grad_similarity_ema = ema_alpha * cosine_sim + (1 - ema_alpha) * grad_similarity_ema
                        
                        # Adjust accumulation steps based on similarity
                        # High similarity (>0.7) means stable gradients -> use fewer accumulation steps
                        # Low similarity (<0.5) means noisy gradients -> use more accumulation steps
                        if grad_similarity_ema > 0.7 and current_accum_steps > min_accum_steps:
                            current_accum_steps = max(min_accum_steps, current_accum_steps - 1)
                        elif grad_similarity_ema < 0.5 and current_accum_steps < max_accum_steps:
                            current_accum_steps = min(max_accum_steps, current_accum_steps + 1)
                
                # Store current gradient for next comparison
                prev_grad_vec = grad_vec.clone()
        ...
        print0(
            f"step:{step}/{args.num_iterations} | loss {lossf:.6f} | accum_steps:{current_accum_steps} | train_time:{approx_training_time_ms/1000:.2f}s | step_avg:{approx_training_time_ms/(step+1):.2f}ms"
        )
        # log to logile
