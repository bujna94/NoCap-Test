...
INTERMEDIATE_VAL_EVERY = 2048
LOG_EVERY = 128
VOCAB_SIZE_PADDED = 50304  # 50257 padded to multiple of 128 for tensor cores
...
if __name__ == "__main__":
    ...
    # Use padded vocab for tensor core alignment
    ...
    for step in range(args.num_iterations + 1):
        ...
        # Determine whether to validate at this step
        do_val = False
        is_final = last_step
        if is_final:
            do_val = True
        ...
        if do_val:
            ...
            if is_final:
                # Final step: evaluate both raw and EMA, report better
                val_loader.reset()
                with torch.no_grad():
                    val_loss_raw = 0.0
                ...
                # Swap back raw weights
                for p, raw_p in zip(raw_model.parameters(), raw_params_backup):
                    p.data.copy_(raw_p)

                val_loss = min(val_loss_raw.item(), val_loss_ema.item())
                print0(f"step:{step}/{args.num_iterations} | val loss {val_loss:.6f} (raw={val_loss_raw.item():.6f}, ema={val_loss_ema.item():.6f})")
                if master_process and logfile is not None:
                    with open(logfile, "a") as f:
                        f.write("s:%d val:%f\n" % (step, val_loss))
            else:
                # Intermediate: use reduced batches with raw model only
                val_loader.reset()
                actual_val_steps = INTERMEDIATE_VAL_BATCHES
                with torch.no_grad():
                    val_loss = 0.0
                ...
                print0(f"step:{step}/{args.num_iterations} | val loss {val_loss:.6f} (approx, {actual_val_steps} batches)")
                if master_process and logfile is not None:
                    with open(logfile, "a") as f:
                        f.write("s:%d val:%f\n" % (step, val_loss))
        ...
        # Sparse logging to reduce CUDA syncs
        if step % LOG_EVERY == 0 or step == args.num_iterations - 1:
            torch.cuda.synchronize()
