...
class Block(nn.Module):
    ...
    def forward(self, x):
        # Attention path
        attn_input = rmsnorm(x)
        if self.skip_backward:
            with torch.no_grad():
                attn_out = self.attn(attn_input)
            attn_out = attn_out.detach().requires_grad_(False)
        else:
            attn_out = self.attn(attn_input)
        ...
        if self.skip_backward:
            ...
            mlp_out = mlp_out.detach().requires_grad_(False)
        else:
            mlp_out = self.mlp(mlp_input)
        x = x + mlp_out
        return x
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Tracking for gradient monitoring
        self.grad_norms = [0.0] * config.n_layer
        self.grad_ema_alpha = 0.9
    ...
    def update_skip_decisions(self, step, warmup_steps=1000):
        """Decide which layers to skip backward for next microbatch based on gradient norms"""
        if step < warmup_steps:
            # Don't skip during warmup
            for block in self.transformer.h:
                block.skip_backward = False
            return
        
        # After warmup, skip layers with very low gradient contribution
        # Use adaptive threshold based on mean gradient norm
        if len(self.grad_norms) > 0 and max(self.grad_norms) > 0:
            mean_grad = sum(self.grad_norms) / len(self.grad_norms)
            threshold = 0.1 * mean_grad  # Skip if grad norm < 10% of mean
            
            for i, block in enumerate(self.transformer.h):
                if i < len(self.grad_norms):
                    block.skip_backward = self.grad_norms[i] < threshold
    
    def collect_gradient_stats(self):
        """Collect gradient norms for each layer after backward pass"""
        for i, block in enumerate(self.transformer.h):
            total_grad_norm = 0.0
            param_count = 0
            ...
            if param_count > 0:
                layer_grad_norm = math.sqrt(total_grad_norm)
                # EMA update
                if self.grad_norms[i] == 0.0:
                    self.grad_norms[i] = layer_grad_norm
                else:
                    self.grad_norms[i] = (
                        self.grad_ema_alpha * self.grad_norms[i] + 
                        (1 - self.grad_ema_alpha) * layer_grad_norm
                    )
...
if __name__ == "__main__":
    ...
    for step in range(args.num_iterations + 1):
        ...
        for micro_step in range(args.grad_accumulation_steps):
            # Update skip decisions before each microbatch
            raw_model.update_skip_decisions(step, warmup_steps=1000)
            ...
            # Collect gradient stats after backward
            if micro_step == args.grad_accumulation_steps - 1:
                raw_model.collect_gradient_stats()
