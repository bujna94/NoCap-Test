...
class GPT(nn.Module):
    ...
    def configure_optimizers(self, weight_decay, learning_rate, betas, device_type):
        # Component-specific momentum: group parameters by component type
        param_groups = []
        
        # Group 1: Embeddings (sparse gradients) - higher beta1 for more momentum
        embedding_params = [p for n, p in self.named_parameters() if 'wte' in n]
        if embedding_params:
            param_groups.append({
                'params': embedding_params,
                'lr': learning_rate,
                'betas': (0.92, 0.95),  # Higher beta1 for sparse embeddings
                'weight_decay': weight_decay
            })
        
        # Group 2: Attention projections (correlated gradients) - standard momentum
        attn_params = [p for n, p in self.named_parameters() if 'attn' in n and 'wte' not in n and 'lm_head' not in n]
        if attn_params:
            param_groups.append({
                'params': attn_params,
                'lr': learning_rate,
                'betas': (0.90, 0.95),  # Standard for attention
                'weight_decay': weight_decay
            })
        
        # Group 3: MLP layers (dense, noisy gradients) - lower beta1 for faster adaptation
        mlp_params = [p for n, p in self.named_parameters() if 'mlp' in n]
        if mlp_params:
            param_groups.append({
                'params': mlp_params,
                'lr': learning_rate,
                'betas': (0.88, 0.95),  # Lower beta1 for noisy MLP gradients
                'weight_decay': weight_decay
            })
        
        # Group 4: Output head (extremely sparse) - highest beta1
        head_params = [p for n, p in self.named_parameters() if 'lm_head' in n and p.requires_grad and p not in embedding_params]
        if head_params:
            param_groups.append({
                'params': head_params,
                'lr': learning_rate,
                'betas': (0.92, 0.95),  # High beta1 for sparse output head
                'weight_decay': weight_decay
            })
        
        # Fallback: any remaining parameters use default
        all_grouped_params = set()
        for group in param_groups:
            all_grouped_params.update(group['params'])
        remaining_params = [p for p in self.parameters() if p not in all_grouped_params and p.requires_grad]
        if remaining_params:
            param_groups.append({
