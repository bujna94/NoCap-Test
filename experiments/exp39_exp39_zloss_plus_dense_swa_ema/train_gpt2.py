...
# Z-loss coefficient - very small to avoid disrupting training
ZLOSS_COEFF = 1e-4

# SWA/EMA settings (proven from exp38)
SWA_START_STEP_OFFSET = 1024  # start collecting snapshots this many steps before end
SWA_SNAPSHOT_EVERY = 8  # collect a snapshot every N steps
EMA_DECAY = 0.998
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, return_logits=True):
        ...
        if targets is not None:
            ...
            # This is log(sum(exp(logits)))^2 averaged over all tokens
            if ZLOSS_COEFF > 0:
                log_z = torch.logsumexp(logits.view(-1, logits.size(-1)), dim=-1)
                z_loss = ZLOSS_COEFF * (log_z ** 2).mean()
                loss = loss + z_loss
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--warmup_iters", type=int, default=0,
    )
    parser.add_argument(
        "--warmdown_iters",
        type=int,
    ...
    # ---------- EMA and SWA Setup ----------
    # EMA: maintain exponential moving average of parameters
    ema_params = {}
    for name, param in raw_model.named_parameters():
        ema_params[name] = param.data.clone()

    # SWA: collect snapshots in the final phase
    swa_start = args.num_iterations - SWA_START_STEP_OFFSET  # step at which to start collecting
    swa_snapshots = []
    swa_count = 0
    ...
    for step in range(args.num_iterations + 1):
        ...
        if args.val_loss_every > 0 and (step % args.val_loss_every == 0 or last_step):
            ...
            with torch.no_grad():
                ...
                for _ in range(val_steps):
                    ...
                    val_loss_original += loss
                dist.all_reduce(val_loss_original, op=dist.ReduceOp.AVG)
                val_loss_original /= val_steps

            best_val_loss = val_loss_original.item()
            ...
            # Try SWA weights if we have snapshots
            if len(swa_snapshots) > 0:
                # Compute average of snapshots
                avg_params = {}
                for name in swa_snapshots[0]:
                    avg_params[name] = torch.zeros_like(swa_snapshots[0][name])
                    for snap in swa_snapshots:
                        avg_params[name] += snap[name]
                    avg_params[name] /= len(swa_snapshots)
            ...
            print0(f"step:{step}/{args.num_iterations} | val loss {val_loss:.6f} | method:{best_method}")
            if master_process:
                if args.log_wandb:
        ...
        if step >= swa_start and (step - swa_start) % SWA_SNAPSHOT_EVERY == 0:
            snapshot = {}
            for name, param in raw_model.named_parameters():
